...
class outTupleWjets():
    ...
    def __init__( self, fileName, era, doSyst=False, shift=[], isMC=True, onlyNom=False, isW=False):
        ...
        self.list_of_arraysJetsNbtagM2p4 = []
        ...
        self.cat = array('I', [0])
        self.topPt_1           = array('f',[0])
        self.topPt_2           = array('f',[0])
        self.topPt_3           = array('f',[0])
        ...
        branches += ['METCor_T1', 'PuppiMETCor', 'METCor_T1Smear'] 
        if not isMC:
            branches= ['MET', 'MET_T1', 'METCorGood_T1', 'PuppiMET', 'PuppiMETCorGood']
            branches += ['METCor_T1', 'PuppiMETCor']
        ...
        branches += ['METCorboson','METCorSmearboson','PuppiMETCorboson']
        ...
        if not isMC:
            branches = ['MET_T1','PuppiMET','METCorGood_T1','PuppiMETCorGood']
            branches += ['METCor_T1','PuppiMETCor']
        ...
        self.t.Branch(
            'Electron_convVeto',
            self.Electron_convVeto,
            'Electron_convVeto/F')
        self.t.Branch(
            'Electron_lostHits',
            self.Electron_lostHits,
            'Electron_lostHits/I')
        ...
        self.t.Branch('topPt_1',           self.topPt_1,            'topPt_1/F' )
        self.t.Branch('topPt_2',           self.topPt_2,            'topPt_2/F' )
        self.t.Branch('topPt_3',           self.topPt_3,            'topPt_3/F' )
        ...
        self.t.Branch('nbtagM2p4', self.nbtagM2p4, 'nbtagM2p4/i')
        ...
        if doSyst:
            ...
            for i, v in enumerate(self.allsystJets):
                ...
                self.t.Branch( 'nbtagM2p4{0:s}'.format(v), self.list_of_arraysJetsNbtagM2p4[i], 'nbtagM2p4{0:s}/i'.format(v))
    ...
    def getJetsJMEMV(self, entry, LepList, era, syst, proc):
        jetList, jetListFlav, jetListPhi, jetListEta, jetListPt, bTagListDeep, bJetListL, bJetListM, bJetListM2p4,bJetListT, bJetListFlav = [], [], [], [], [], [], [], [], [], [], []
        isHEM = 0
        num_elements = 8
        default_value = -99
        ...
        # print '' # print jetList, jetListEta, jetListPt
        # return jetList, jetListFlav, jetListEta,  jetListPt, bTagListDeep,
        # bJetListL,bJetListM,bJetListT,bJetListFlav
        #print ('btag', len(jetList), bJetListL, bJetListM, bJetListM2p4)
        return jetList, jetListFlav, jetListPhi, jetListEta, jetListPt, bTagListDeep, bJetListL, bJetListM, bJetListM2p4, bJetListT, bJetListFlav, isHEM
    ...
    def FillW(
        ...
        if SystIndex == 0:
            ...
            if len(hltListLep) > 0 and len(hltListLepSubL) > 0:
                is_trig_1 = 3
            # for 2016 UL, leadL= IsoMu24, subLeadL = IsoTkMu24 should give 2
            # if both fire,

            # self.whichTriggerWord[0]=0
            # self.whichTriggerWordSubL[0]=0

            #if len(TrigListLep) > 0: 
            #    print('TrigerList ===========>', TrigListLep, lepList, hltListLep, channel_ll, 'istrig_1', is_trig_1, 'istrig_2', is_trig_2, 'lenTrigList', len(TrigListLep), 'lenLept', len(lepList), 'lepList_0', lepList[0], 'TrigList_0', TrigListLep[0], hltListLep)
        ...
        self.topPt_1[0]           = exp(0.0615-0.005*Lep.Pt())
        self.topPt_2[0]           = exp(0.0416-0.003*Lep.Pt())
        self.topPt_3[0]           = 0.103* exp(-0.0118*Lep.Pt())-0.000134*Lep.Pt()+0.973
        muonid = 1.
        muoniso = 1.
        muontrig = 1.
        eleid = 1.
        ...
        # print 'cat', cat, channel_ll, self.pt_1[0], self.eta_1[0],
        # self.isTrig_1[0]
        leplist = []
        leplist.append(Lep)
        ...
        if channel_ll == 'enu':
            ...
            self.Electron_lostHits[0] = (                entry.Electron_lostHits[lep_index_1])
        ...
        if channel_ll == 'mnu':
            ...
            self.PFiso_Id_1 = entry.Muon_pfIsoId[lep_index_1]

            self.miniiso_1 = entry.Muon_miniPFRelIso_all[lep_index_1]
            self.miniiso_Id_1 = entry.Muon_miniIsoId[lep_index_1]
        ...
        if SaveEventMu or SaveEventEl:
            ...
            if 'UL' in proc:
                ...
                # print self.u_par_RawPuppiMET[0], self.u_perp_RawPuppiMET[0],
                # self.RawPuppiMET_pt[0], self.RawPuppiMET_phi[0]
                ...
                #print ('cor vs corggod pt', met[2], mett[2], (met[2]-mett[2])/mett[2], 'phi ', met[3], mett[3], (met[3]-mett[3])/mett[3])
                ...
                if doUncertainties:
                    ...
                    main_branches += ['METCor_T1', 'METCor_T1Smear','PuppiMETCor']

                    if not isMC:
                        main_branches = ['MET_T1', 'METCorGood_T1', 'PuppiMET', 'PuppiMETCorGood']
                        main_branches += ['METCor_T1', 'PuppiMETCor']
                    ...
                    main_branches += ['METCorboson', 'METCorSmearboson','PuppiMETCorboson']
                    if not isMC:
                        main_branches = ['METCorGoodboson', 'PuppiMETCorGoodboson']
                        main_branches += ['METCorboson', 'PuppiMETCorboson']
                    ...
                    for branch in main_branches:
                        for systematic in systematics:
                            ...
                            met_ = branch[:-5]
                            if 'Puppi' not in met_ and 'Smear' not in met_:
                                met_ += '_T1'
                            if 'Smear' in met_:
                            ...
                            # print 'will get that ',
                            # "{}_pt{}".format(met_,systematic), " for ", met_,
                            # systematic
                            ...
                            #delta_phi = lV.DeltaPhi(metV)

                            #if systematic =='' : print (' dphis', dPhi, delta_phi, sqrt (  2 * metV.Pt() * lV.Pt()   * ( 1-  cos(dPhi))) , sqrt (  2 * metV.Pt() * lV.Pt()   * ( 1-  cos(delta_phi))), ( metV + lV).Mt()  )
                            #transmt = sqrt (  2 * metV.Pt() * lV.Pt()   * ( 1-  cos(dPhi)))
                            #delta_phi = lV.Phi() - metV.Phi()
                            #delta_phi = ROOT.TMath.Abs(ROOT.TMath.Pi() - ROOT.TMath.Abs(ROOT.TMath.Pi() - delta_phi))  # Correct for the periodic nature
                            #if systematic =='' : print ('transsssss', ( metV + lV).Mt(), transmt, systematic, dPhi ,dPhi2, 'ROOT dPhi', delta_phi)
                            #if 'Puppi' not in branch:
            ...
            if 'UL' in proc or '2016' not in str(era):
                ...
                try:
                    self.Flag_BadPFMuonDzFilter[0] = int( entry.Flag_BadPFMuonDzFilter)
                except AttributeError:
                    self.Flag_BadPFMuonDzFilter[0] = 1
            ...
            if doUncertainties:
                ...
                for i, v in enumerate(self.allsystJets):
                    ...
                    jetList, jetListFlav, jetListPhi, jetListEta, jetListPt, bTagListDeep, bJetListL, bJetListM, bJetListM2p4, bJetListT, bJetListFlav, isHEM = self.getJetsJMEMV( entry, leplist, era, str(v), proc)
                    try:
                        ...
                        self.list_of_arraysJetsNbtagM2p4[i][0] = len(bJetListM2p4)
                    ...
                    for ifl in range(len(jetList)):
                        try:
                            ...
                            print( 'hit the ceiling', len(jetListPt), 'event', entry.event, 'lumi', entry.luminosityBlock, 'run', entry.run)
                    if 'nom' in v:
                        self.njets[0], self.nbtagL[0], self.nbtagM[0], self.nbtagM2p4[0],self.nbtagT[0] = -1, -1, -1, -1,-1
                        self.isHEM[0] = 0
                        self.isHEM[0] = isHEM
                        self.njets[0] = len(jetList)
                        self.nbtagL[0] = len(bJetListL)
                        self.nbtagM[0] = len(bJetListM)
                        self.nbtagM2p4[0] = len(bJetListM2p4)
                        ...
                        for ifl in range(len(jetListPt)):
                            try:
                                ...
                                print( 'we hit the ceiling', len(jetListPt), 'event', entry.event, 'lumi', entry.luminosityBlock, 'run', entry.run)
		...
		for var_suffix in variations:
            ...
            '''
            for branch in self.t.GetListOfBranches():
                branch_name = branch.GetName()
                if "CorGood" in branch_name:
                    self.t.SetBranchStatus(branch_name, 0)
            '''
