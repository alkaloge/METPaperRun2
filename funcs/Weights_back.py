...
class Weights() :
	...
	for i, v in enumerate(allmets) :
	    ...
	    try : 
                ...
                #tryV2.SetPx(j*cos(jphi))
                #tryV2.SetPy(j*sin(jphi))
                
                tryV += (leptV-leptV*corr)
                fcor = (leptV.Pt()-leptV.Pt()*corr)
                #tryV2 = tryV2 + (leptV-leptV*corr)
                ...
                if i==0 or i==24 or i==25 : print 'inside all MET',  i, v, 'nomMET', entry.MET_T1_pt, 'sysMETuncor', j, 'sysMET_cor', tryV.Pt(), corr, fcor, entry.event, entry.luminosityBlock, entry.run
    ...
    #def applyES(self,entry, year, systematic,  metPtPhi,  printOn=False) :
		...
		if gen_match == 5 :
		    ...
		    if 'Down' in systematic : dirr = 'Down'
                    ...
                    if systematic == 'Central' : print 'MET inside the loop', self.MetV.Pt(), 'tau', j, 'from', entry.nTau, gen_match, entry.event
                    self.MetV +=( self.LeptV - self.LeptV*tes)
                    if systematic == 'Central' : print 'MET corrected inside the loop', self.MetV.Pt(), 'tau', j, 'from', entry.nTau, gen_match, entry.event
