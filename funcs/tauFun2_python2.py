...
def getTauList(
    ...
    for j in range(entry.nTau):
        ...
        # print ord(entry.Tau_idDeepTau2017v2p1VSmu[j]),
        # ord(entry.Tau_idDeepTau2017v2p1VSe[j]),
        # ord(entry.Tau_idDeepTau2017v2p1VSjet[j])
        ...
        for j in range(entry.nElectron):
            ...
            if em['ele_ID']:
                if not entry.Electron_mvaFall17V2noIso_WP90[j]:
                    if printOn:
                        print((
                            "        failed mvaWP90={0}".format(
                                entry.entry.Electron_mvaFall17V2noIso_WP90[j])))
                    continue
            if em['ele_iso_f'] and entry.Electron_pfRelIso03_all[j] > em['ele_iso']:
                if printOn:
                    print((
                        "        failed iso={0:f}".format(
                            entry.Electron_pfRelIso03_all[j])))
                continue
...
def goodMuonWjets(entry, j):
    ...
    if mm['mu_iso_f'] and entry.Muon_pfRelIso04_all[j] > mm['mu_iso']:
        return False
    if mm['mu_ID']:
        # if not (entry.Muon_tightId[j]) or ord(entry.Muon_pfIsoId[j]) < 4:
        # return False
...
def goodElectron(entry, j):
    ...
    if abs(entry.Electron_dz[j]) > ee['ele_dz']:
        return False
    if ord(entry.Electron_lostHits[j]) > ee['ele_lostHits']:
        return False
...
def vetoElectron(entry, j):
    ...
    # ord(entry.Electron_lostHits[j]), 'iso :',
