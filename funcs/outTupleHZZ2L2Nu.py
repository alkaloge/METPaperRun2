...
class outTupleHZZ2L2Nu() :
	...
	self.tauMass = 1.7768 
		...
		for jes in self.jessyst :
			...
			self.list_of_arraysDiJetsMass.append( array('f',[-9.99]*10))
			self.list_of_arraysJetsPt.append( array('f',[-9.99]*10))
			self.list_of_arraysdPhiJetPFMET.append( array('f',[-9.99]*10))
			self.list_of_arraysdPhiJetPuppiMET.append( array('f',[-9.99]*10))
			self.list_of_arraysJetsMass.append( array('f',[-9.99]*10))
	...
	self.u_perp_MET = array('f',[-999])
        ...
        self.jpt     = array('f',[-9.99]*10)
        self.dphijmet     = array('f',[-9.99]*10)
        self.dphijpuppimet     = array('f',[-9.99]*10)
        self.jisbtagL     = array('f',[-9.99]*10)
        self.jisbtagM     = array('f',[-9.99]*10)
        self.jisbtagT     = array('f',[-9.99]*10)
        self.jmass     = array('f',[-9.99]*10)
        self.dijmass    = array('f',[-9.99]*10)
	...
	if channel_ll == 'ee' : 
            ...
            self.Electron_lostHits_1[0]  = ord(entry.Electron_lostHits[lep_index_1])
            self.Electron_convVeto_2[0]  = entry.Electron_convVeto[lep_index_2]
            self.Electron_lostHits_2[0]  = ord(entry.Electron_lostHits[lep_index_2])
	...
	if '2016' in era:
		...
		try : 
		    ...
		    mett = correctedMET(entry.MET_pt, entry.MET_phi, entry.PV_npvsGood, entry.run, isMC, yearin, True, False)
		...
		self.METCorGood_T1_phi[0]= mett[3]
		    ...
		    try : metts = correctedMET(entry.MET_T1Smear_pt, entry.MET_T1Smear_phi, entry.PV_npvsGood, entry.run, isMC, yearin, True, False)
                    except AttributeError :  metts = correctedMET(entry.MET_pt, entry.MET_phi, entry.PV_npvsGood, entry.run, isMC, yearin, True, False)
		...
		for i, v in enumerate(self.allsystJets) : 
			...
			try : 
			    ...
			    self.list_of_arraysDiJetsMass[i][ifl] = jetDiMass[ifl]
			    self.list_of_arraysJetsPt[i][ifl] = jetListPt[ifl]
			    self.list_of_arraysdPhiJetPFMET[i][ifl] = dPhiJetPFMET[ifl]
			    self.list_of_arraysdPhiJetPuppiMET[i][ifl] = dPhiJetPuppiMET[ifl]
			...
			for i in range(len(self.jpt)):
			    self.jpt[i] = -9.99
			    self.jisbtagL[i] = -9.99
			    self.jisbtagM[i] = -9.99
			    self.jisbtagT[i] = -9.99
			    self.jmass[i] = -9.99
			    self.dijmass[i] = -9.99
			    self.jeta[i] = -9.99
			    self.jphi[i] = -9.99
			...
			for ifl in range(len(jetListPt)) :
				...
				self.dijmass[ifl]  = jetDiMass[ifl]
