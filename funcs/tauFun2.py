...
def goodTrigger(e, year):
    ...
    if not (trig['singleLepton'] or trig['doubleLepton']):
        return True
    ...
    # single ele 2018:  HLT Ele32 WPTight Gsf v, HLT Ele35 WPTight Gsf v and
    # cut pt(ele)>33, eta(ele)<2.1
    ...
    return (trig['singleLepton'] and goodSingle) or (
        trig['doubleLepton'] and goodDouble)
...
def getTauList(
    ...
    if channel not in ['mmtt', 'eett']:
        print(
            "Warning: invalid channel={0:s} in tauFun.getTauList()".format(channel))
        exit()

    if printOn:
        print(' getTauList : will be checking nTau', entry.nTau)

    if entry.nTau == 0:
        if printOn:
            print(' failed nTau', entry.nTau)
        return []
    ...
    for j in range(entry.nTau):
        ...
        if printOn:
            print('looking for Tau j', j, 'Q', entry.Tau_charge[j])

        if signC != 0 and entry.Tau_charge[j] == signC:
            if printOn:
                print(
                    "        fail same Tau Q {0:d} as lepton Q".format(
                        entry.Tau_charge[j]))
            continue

        if entry.Tau_pt[j] < tt['tau_pt']:
            if printOn:
                print("        fail Tau pT {0:f}".format(entry.Tau_pt[j]))
            continue
        if abs(entry.Tau_eta[j]) > tt['tau_eta']:

            if printOn:
                print("        fail Tau eta {0:f}".format(
                    abs(entry.Tau_eta[j])))
            continue
        if abs(entry.Tau_dz[j]) > tt['tau_dz']:
            if printOn:
                print(("        fail Tau dZ {0:f}".format(entry.Tau_dz[j])))
            continue
        if not entry.Tau_idDecayModeNewDMs[j]:
            if printOn:
                print((
                    "        fail Tau decayModeNewDM {0:f}".format(
                        entry.Tau_idDecayModeNewDMs[j])))
            continue
        if entry.Tau_decayMode[j] == 5 or entry.Tau_decayMode[j] == 6:
            if printOn:
                print((
                    "        fail Tau decayMode {0:f}".format(
                        entry.Tau_decayMode[j])))
            continue
        if abs(entry.Tau_charge[j]) != 1:
            if printOn:
                print(("        fail Tau Q {0:d}".format(entry.Tau_charge[j])))
            continue

        if tt['tau_vJet'] > 0 and not (
                entry.Tau_idDeepTau2017v2p1VSjet[j]) & tt['tau_vJet'] > 0:
            if printOn:
                print(("        fail DeepTau vs. Jet={0:d}".format(
                    (entry.Tau_idDeepTau2017v2p1VSjet[j]))))
            continue
        if tt['tau_vEle'] > 0 and not (
                entry.Tau_idDeepTau2017v2p1VSe[j]) & tt['tau_vEle'] > 0:
            if printOn:
                print(("        fail DeepTau vs. ele={0:d}".format(
                    (entry.Tau_idDeepTau2017v2p1VSe[j]))))
            continue
        if tt['tau_vMu'] > 0 and not (
                entry.Tau_idDeepTau2017v2p1VSmu[j]) & tt['tau_vMu'] > 0:
            if printOn:
                print(("        fail DeepTau vs.  mu={0:d}".format(
                    (entry.Tau_idDeepTau2017v2p1VSmu[j]))))
            continue
        ...
        if isTight:
            if DR0 < tt['tt_DR'] or DR1 < tt['tt_DR']:
                ...
                if printOn:
                    print((
                        "        fail DR0={0:f} or DR1={1:f}". format(
                            DR0, DR1)))
                continue
        tauList.append(j)
        print("charge of taus ->sign of opposite one ", signC, "charge of Tau", entry.Tau_charge[j], "idx", j)
        # print (entry.Tau_idDeepTau2017v2p1VSmu[j]),
        # (entry.Tau_idDeepTau2017v2p1VSe[j]),
        # (entry.Tau_idDeepTau2017v2p1VSjet[j])

    if printOn:
        print('returning with tauList from getTauList', tauList)
    return tauList
...
def getGoodTauListWjets(channel, entry, muIndex, printOn=False):
    ...
    for j in range(entry.nTau):
        # apply tau(h) selections
        if channel == 'mnu' and llDR(entry.Tau_eta[j], entry.Tau_phi[j], eta2, phi2) > 0.2 or int(
                (entry.Tau_idDeepTau2017v2p1VSmu[j])) > 0:
            # print 'failed dR or antiMu', llDR(entry.Tau_eta[j],
            # entry.Tau_phi[j], eta2, phi2),  entry.Tau_pt[j], 'antiMu',
            # int((entry.Tau_idDeepTau2017v2p1VSmu[j]))
            tauList.append(j)
        if channel == 'enu' and llDR(entry.Tau_eta[j], entry.Tau_phi[j], eta2, phi2) > 0.2 or int(
                (entry.Tau_idDeepTau2017v2p1VSe[j])) > 0:
            # print 'failed dR or antiMu', llDR(entry.Tau_eta[j],
            # entry.Tau_phi[j], eta2, phi2),  entry.Tau_pt[j], 'antiMu',
            # int((entry.Tau_idDeepTau2017v2p1VSmu[j]))
            tauList.append(j)

        # if tt['tau_vJet'] > 0  and not (entry.Tau_idDeepTau2017v2p1VSjet[j]) & tt['tau_vJet'] > 0 :
        #    if printOn : print("        fail DeepTau vs. Jet={0:d}".format((entry.Tau_idDeepTau2017v2p1VSjet[j])))
        #    continue
        # if tt['tau_vEle'] > 0 and not (entry.Tau_idDeepTau2017v2p1VSe[j]) & tt['tau_vEle'] > 0 :
        #    if printOn : print("        fail DeepTau vs. ele={0:d}".format((entry.Tau_idDeepTau2017v2p1VSe[j])))
        #    continue
    '''
    if len(tauList) > 0 :
	for jj in range(len(tauList)):
	     j= tauList[jj]
	     print 'has a tau not coming from mu', len(tauList), jj, entry.nTau, entry.Tau_pt[j], entry.Tau_phi[j], entry.Tau_dz[j], entry.Tau_idDecayModeOldDMs[j], entry.Tau_decayMode[j], 'Flv', (entry.Tau_genPartFlav[j]), 'antiDead',  entry.Tau_idAntiEleDeadECal[j], 'antiEl', (entry.Tau_idDeepTau2017v2p1VSe[j]), 'antiMu', (entry.Tau_idDeepTau2017v2p1VSmu[j]), 'antiJet', (entry.Tau_idDeepTau2017v2p1VSjet[j]), 'lDR', llDR(entry.Tau_eta[j], entry.Tau_phi[j], eta2, phi2), entry.Muon_pt[muIndex], entry.Muon_phi[muIndex]
        print ''
    '''
    return tauList
...
def mllCut(mll):
    ...
    if mll < mllcuts['mll_low'] or mll > mllcuts['mll_high']:
        return False
    return True
...
def getTauPointer(entry, eta1, phi1):
    ...
    for j in range(entry.nTau):
        ...
        if DR < bestMatch:
            bestMatch, jBest = DR, j
    if bestMatch > 0.1:
        jBest = -1
        print((
            "Error in getTauPointer():   No match found eta={0:.3f} phi={1:.3f}".format(
                eta1,
                phi1)))
    return jBest
...
def comparePairvspT(entry, tauPairList, printOn=False):
    ...
    for i in range(0, len(tauPairList)):
        ...
        # look at leading pt tau in each pair
        j1, j2 = tauPairList[i][0], tauPairList[i][1]
        if printOn:
            print('appending now', entry.Tau_pt[j1] + entry.Tau_pt[j2], j1, j2, tauPairList[i])
    ...
    if entry.Tau_rawDeepTau2017v2p1VSjet[j2] == entry.Tau_rawDeepTau2017v2p1VSjet[j1]:
        ...
        if entry.Tau_pt[j2] == entry.Tau_pt[j1]:
            ...
            if entry.Tau_rawDeepTau2017v2p1VSjet[j4] < entry.Tau_rawDeepTau2017v2p1VSjet[j3]:
                return False
            if entry.Tau_rawDeepTau2017v2p1VSjet[j4] == entry.Tau_rawDeepTau2017v2p1VSjet[j3]:
                if entry.Tau_pt[j4] > entry.Tau_pt[j3]:
                    return True
                if entry.Tau_pt[j4] < entry.Tau_pt[j3]:
                    return False
                if entry.Tau_pt[j4] == entry.Tau_pt[j3]:
...
def getBestTauPair(channel, entry, tauList, printOn=False):
    ...
    if channel not in ['mmtt', 'eett']:
        if printOn:
            print((
                "Invalid channel={0:s} in tauFun.getBestTauPair()".format(channel)))
        exit()

    if len(tauList) < 2:
        if printOn:
            print('failed to find good TauTau Pair')
        if printOn:
            print((
                "Entering getTauPairs failing nTauList={0:s}".format(tauList)))
        return []
    ...
    for i in range(len(tauList)):
        ...
        for j in range(len(tauList)):
            ...
            if tauDR(entry, idx_tau1, idx_tau2) < tt['tt_DR']:
                ...
                if printOn:
                    print("failed tDR=", tdR, "tt_DR =", tt['tt_DR'])
                continue
            #tauPairList.append([idx_tau1, idx_tau2])
            if idx_tau1 not in tauPairList:
                tauPairList.append(idx_tau1)
            if idx_tau2 not in tauPairList:
                tauPairList.append(idx_tau2)

    if len(tauPairList) == 0:
        if printOn:
            print('fail tauPairList =0')
        return []

    c = set(combinations(tauPairList, 2))
    if printOn:
        print('these are the combinations', c, 'from', tauPairList)
    tauPairList = list(c)

    if printOn:
        print('Do I need to do the comparePair ? ', len(tauPairList), tauPairList)

    maxI = comparePairvspT(entry, tauPairList, printOn)
    tauPairList = tauPairList[maxI]

    if printOn:
        print('this is the list of sums', maxI, tauPairList, tauPairList[0], tauPairList[1])

    idx_tau1, idx_tau2 = tauPairList[0], tauPairList[1]
    if entry.Tau_pt[idx_tau2] > entry.Tau_pt[idx_tau1]:
        if printOn:
            print('Sorting tt tauPair', entry.Tau_pt[idx_tau2], entry.Tau_pt[idx_tau1], idx_tau2, idx_tau1)
        tauPairList = {}
        tauPairList[0] = idx_tau2
        tauPairList[1] = idx_tau1

    if printOn:
        print('returning tt tauPairList', tauPairList)
    return tauPairList
                    ...
                    if entry.Muon_pt[i] > mt['mu_pt']:
                        if abs(entry.Muon_eta[i]) < mt['mu_eta']:
                            if mt['mu_iso_f'] and entry.Muon_pfRelIso04_all[i] < mt['mu_iso']:
                                ...
                                if mt['mu_type'] and (
                                        entry.Muon_isGlobal[j] or entry.Muon_isTracker[j]):
                                    if mt['mu_ID'] and entry.Muon_mediumId[j]:
                                        if abs(
                                                entry.Muon_dxy[j]) < mt['mu_dxy']:
                                            if abs(
                                                    entry.Muon_dz[j]) < mt['mu_dz']:
                                                if entry.Muon_pt[j] > mt['mu_pt']:
                                                    if abs(
                                                            entry.Muon_eta[j]) < mt['mu_eta']:
                                                        if mt['mu_iso_f'] and entry.Muon_pfRelIso04_all[j] < mt['mu_iso']:
                                                            if printOn:
                                                                print('checking overlap....', i, j, DRobj(entry.Muon_eta[i], entry.Muon_phi[i], entry.Muon_eta[j], entry.Muon_phi[j]), entry.Muon_pt[i], entry.Muon_eta[i], entry.Muon_phi[i], 'j-->', entry.Muon_pt[j], entry.Muon_eta[j], entry.Muon_phi[j])
                                                            if checkDR:
                                                                if DRobj(
                                                                        entry.Muon_eta[i],
                                                                        entry.Muon_phi[i],
                                                                        entry.Muon_eta[j],
                                                                        entry.Muon_phi[j]) < mt['ll_DR']:
                                                                    overlapM = True
                                                            else:
                                                                overlapM = True
    ...
    if (cat == 'mmmt' and entry.nMuon > 3) or (
        ...
        for i in range(entry.nMuon):
            ...
            for j in range(i + 1, entry.nMuon):
                # if printOn : print 'checking in loop pT with findZ nMuon',
                # entry.nMuon, 'j', j, entry.run, entry.luminosityBlock,
                # entry.event, pairList[0].Pt(), pairList[1].Pt(), 'vs',
                # entry.Muon_pt[j]
                if DRobj(
                        entry.Muon_eta[j],
                        entry.Muon_phi[j],
                        ...
                        pairList[1].Eta(),
                        pairList[1].Phi()) < 0.1:
                    # print 'found a match on findZ j now ', j,
                    # entry.Muon_pt[i]
                    continue

                if checkOverlapMuon(entry, i, j):
                    if printOn:
                        print('Muons ', i, j, 'overlaping ---->', entry.Muon_pt[i], entry.Muon_pt[j], DRobj(entry.Muon_eta[i], entry.Muon_phi[i], entry.Muon_eta[j], entry.Muon_phi[j]), entry.run, entry.luminosityBlock, entry.event)
                    return []
    ...
    for i in range(entry.nMuon):
        ...
        if mt['mu_type']:
            if not (entry.Muon_isGlobal[i] or entry.Muon_isTracker[i]):
                if printOn:
                    print((
                        "    fail mu_type Global or Tracker={0}".format(
                            entry.Muon_isGlobal[i])))
                continue
        if mt['mu_ID']:
            if not (entry.Muon_mediumId[i] or entry.Muon_tightId[i]):
                if printOn:
                    print((
                        "    fail mu_ID mediumId={0}".format(
                            entry.Muon_mediumId[i])))
                continue
        if abs(entry.Muon_dxy[i]) > mt['mu_dxy']:
            if printOn:
                print(("    fail mu_dxy={0:f}".format(entry.Muon_dxy[i])))
            continue
        if abs(entry.Muon_dz[i]) > mt['mu_dz']:
            if printOn:
                print(("    fail mu_dz={0:f}".format(entry.Muon_dz[i])))
            continue
        mu_eta, mu_phi = entry.Muon_eta[i], entry.Muon_phi[i]
        if entry.Muon_pt[i] < mt['mu_pt']:
            if printOn:
                print(("    fail mu_pt={0:f}".format(entry.Muon_pt[i])))
            continue
        if abs(mu_eta) > mt['mu_eta']:
            if printOn:
                print(("    fail mu_eta={0:f}".format(entry.Muon_eta[i])))
            continue
        if mt['mu_iso_f'] and entry.Muon_pfRelIso04_all[i] > mt['mu_iso']:
            if printOn:
                print((
                    "    fail mu_iso={0:f}".format(
                        entry.Muon_pfRelIso04_all[i])))
            continue
        ...
        if printOn:
            print('some info----------------', i, entry.Muon_pt[i], entry.Muon_eta[i], entry.Muon_dxy[i], entry.Muon_dz[i], entry.Muon_mediumId[i], entry.Muon_isGlobal[i], entry.Muon_isTracker[i], entry.Muon_pfRelIso04_all[i], DR0, DR1)

        if DR0 < mt['ll_DR'] or DR1 < mt['ll_DR']:
            if printOn:
                print((
                    "    fail muon DR  DR0={0:f} DR1={1:f} for muon={2:s} vs 0.pT={3:f} vs 1.pT={4:f}".format(
                        DR0,
                        DR1,
                        str(i),
                        pairList[0].Pt(),
        ...
        if printOn:
            print((
                "    Good muon i={0:d} lumi={1:s} run={2:s} event={3:s}".format(
                    i, str(
                        entry.luminosityBlock), str(
                        entry.run), str(
                        entry.event)), entry.Muon_pt[i], entry.Muon_mediumId[i], entry.Muon_pfRelIso04_all[i]))

        chargeMu = entry.Muon_charge[i]
        if signC != 0 and entry.Muon_charge[i] == signC:
            continue
        ...
        for j in range(entry.nTau):
            ...
            # apply tau(h) selections
            if printOn:
                print(("        tau j={0:d}".format(j)))
            if abs(entry.Tau_eta[j]) > mt['tau_eta']:
                if printOn:
                    print((
                        "        fail tau eta={0:f}".format(
                            entry.Tau_eta[j])))
                continue
            ...
            if mt['tau_ID']:
                if not entry.Tau_idDecayModeNewDMs[j]:
                    if printOn:
                        print((
                            "        fail tau idDecayModeNewDMs={0}".format(
                                entry.Tau_idDecayModeNewDMs[j])))
                    continue
            if mt['tau_decayMode'] and (
                    entry.Tau_decayMode[j] == 5 or entry.Tau_decayMode[j] == 6):
                if printOn:
                    print((
                        "        fail tau decayMode={0:d}".format(
                            entry.Tau_decayMode[j])))
                continue
	    ...
	    if (entry.Tau_idAntiMu[j]) <= mt['tau_antiMu']: continue
            if (entry.Tau_idAntiEle[j]) <= mt['tau_antiEle']: continue
            if cat == 'eemt':
                if (entry.Tau_idAntiMu[j]) < mt['tau_eemt_antiMu']: continue
	    '''

            if mt['tau_vJet'] > 0 and not (
                    entry.Tau_idDeepTau2017v2p1VSjet[j]) & mt['tau_vJet'] > 0:
                if printOn:
                    print(("        fail DeepTau vs. Jet={0:d}".format(
                        (entry.Tau_idDeepTau2017v2p1VSjet[j]))))
                continue
            if mt['tau_vEle'] > 0 and not (
                    entry.Tau_idDeepTau2017v2p1VSe[j]) & mt['tau_vEle'] > 0:
                if printOn:
                    print(("        fail DeepTau vs. ele={0:d}".format(
                        (entry.Tau_idDeepTau2017v2p1VSe[j]))))
                continue
            if mt['tau_vMu'] > 0 and not (
                    entry.Tau_idDeepTau2017v2p1VSmu[j]) & mt['tau_vMu'] > 0:
                if printOn:
                    print(("        fail DeepTau vs.  mu={0:d}".format(
                        (entry.Tau_idDeepTau2017v2p1VSmu[j]))))
                continue
            ...
            if DR < mt['mt_DR']:
                if printOn:
                    print((
                        "        fail mtDR DR={0:f} for tau={1:d}".format(
                            DR, j)))
                continue
            ...
            if DR0 < mt['mt_DR'] or DR1 < mt['mt_DR']:
                if printOn:
                    print((
                        "        fail DR  DR0={0:f} DR1={1:f} for tau={2:i}, mu={3:i}".format(
                            DR0, DR1, j)))
                continue
            if printOn:
                print(("        Tau j={0:d} passes all cuts.".format(j)))
...
def compareMuTauPairvspT(entry, tauPairList, printOn=False):
    ...
    for i in range(0, len(tauPairList)):
        ...
        j1, j2 = tauPairList[i][0], tauPairList[i][1]
        if printOn:
            print('appending now', entry.Muon_pt[j1] + entry.Tau_pt[j2], j1, j2, tauPairList[i])
...
def getBestMuTauPair(entry, cat='mt', pairList=[], printOn=False, signC=0):
    ...
    # form all possible pairs that satisfy DR requirement
    if printOn:
        print("Entering getBestMuTauPair()")
    tauPairList = getMuTauPairs(
        entry,
        cat=cat,
        pairList=pairList,
        printOn=printOn,
    ...
    if len(tauPairList) == 0:
        if printOn:
            print('failed to find good MuTau Pair', cat)
        return []
...
def getEMuTauPairs(entry, cat='em', pairList=[], printOn=False, signC=0):
    ...
    if printOn:
        print((
            "Entering getEMuTauPairs() nMuon={0:d} nElectron={1:d}".format(
                entry.nMuon,
                entry.nElectron)))
    if entry.nMuon < 1 or entry.nElectron < 1:
        return []
    if cat == 'mmem' and entry.nMuon < 3:
        return []
    if cat == 'eeem' and entry.nElectron < 3:
        return []
    ...
    for i in range(entry.nMuon):
        ...
        if printOn:
            print(("Muon i={0:d}".format(i)))
        if em['mu_ID']:
            if not (entry.Muon_mediumId[i] or entry.Muon_tightId[i]):
                if printOn:
                    print(("    failed muID={0}".format(entry.Muon_mediumId[i])))
                continue
        ...
        if abs(entry.Muon_dxy[i]) > em['mu_dxy']:
            if printOn:
                print(("    failed dxy={0:f}".format(entry.Muon_dxy[i])))
            continue
        if abs(entry.Muon_dz[i]) > em['mu_dz']:
            if printOn:
                print(("    failed dz={0:f}".format(entry.Muon_dz[i])))
            continue
        mu_eta, mu_phi = entry.Muon_eta[i], entry.Muon_phi[i]
        if entry.Muon_pt[i] < em['mu_pt']:
            if printOn:
                print(("    failed pt={0:f}".format(entry.Muon_pt[i])))
            continue
        if abs(mu_eta) > em['mu_eta']:
            if printOn:
                print(("    failed eta={0:f}".format(entry.Muon_eta[i])))
            continue
        if em['mu_iso_f'] and entry.Muon_pfRelIso04_all[i] > em['mu_iso']:
            if printOn:
                print((
                    "    failed iso={0:f}".format(
                        entry.Muon_pfRelIso04_all[i])))
            continue
        ...
        if DR0 < em['ll_DR'] or DR1 < em['ll_DR']:
            if printOn:
                print(("    failed DR  DR0={0:f} DR1={1:f}".format(DR0, DR1)))
            continue
        chargeMu = entry.Muon_charge[i]
        if signC != 0 and entry.Muon_charge[i] == signC:
            continue
        ...
        for j in range(entry.nElectron):
            ...
            if printOn:
                print(("    electron={0:d}".format(j)))
            if abs(entry.Electron_dxy[j]) > em['ele_dxy']:
                if printOn:
                    print((
                        "        failed dxy={0:f}".format(
                            entry.Electron_dxy[j])))
                continue
            if abs(entry.Electron_dz[j]) > em['ele_dz']:
                if printOn:
                    print((
                        "        failed dz={0:f}".format(
                            entry.Electron_dz[j])))
                continue
            ...
            if entry.Electron_pt[j] < em['ele_pt']:
                if printOn:
                    print((
                        "        failed pt={0:f}".format(
                            entry.Electron_pt[j])))
                continue
            if abs(ele_eta) > em['ele_eta']:
                if printOn:
                    print((
                        "        failed eta={0:f}".format(
                            entry.Electron_eta[j])))
                continue
            if (entry.Electron_lostHits[j]) > em['ele_lostHits']:
                if printOn:
                    print((
                        "        failed lost hits={0:d}".format(
                            entry.Electron_lostHits[j])))
                continue
            if em['ele_convVeto']:
                if not entry.Electron_convVeto[j]:
                    if printOn:
                        print((
                            "        failed conv. veto={0}".format(
                                entry.Electron_convVeto[j])))
                    continue
            if em['ele_ID']:
                if not entry.Electron_mvaFall17V2noIso_WP90[j]:
                    if printOn:
                        print(("        failed mvaWP90={0}".format(entry.entry.Electron_mvaFall17V2noIso_WP90[j])))
                    continue
            if em['ele_iso_f'] and entry.Electron_pfRelIso03_all[j] > em['ele_iso']:
                if printOn:
                    print(("        failed iso={0:f}".format(entry.Electron_pfRelIso03_all[j])))
                continue

            if signC != 0 and entry.Electron_charge[j] == signC:
                continue
            ...
            if DR < em['em_DR']:
                if printOn:
                    print(("        failed emDR={0:f}".format(DR)))
                continue
            ...
            if DR0 < em['ll_DR'] or DR1 < em['ll_DR']:
                if printOn:
                    print((
                        "        failed ltDR DR0={0:f} DR1={1:f}".format(
                            DR0, DR1)))
                continue
            if printOn:
                print((
                    "        found a good pair i={0:i}, j={1:i}".format(
                        i, j)))
...
def compareEMuPairvspT(entry, tauPairList, printOn=False):
    ...
    for i in range(0, len(tauPairList)):
        ...
        j1, j2 = tauPairList[i][0], tauPairList[i][1]
        if printOn:
            print('appending now', entry.Electron_pt[j1] + entry.Muon_pt[j2], j1, j2, tauPairList[i])
...
def getBestEMuTauPair(entry, cat, pairList=[], printOn=False, signC=0):
    ...
    if printOn:
        print("Entering getBestEMuTauPair")
    # form all possible pairs that satisfy DR requirement
    tauPairList = getEMuTauPairs(
        entry,
        cat=cat,
        pairList=pairList,
        printOn=printOn,
    ...
    if len(tauPairList) == 0:
        if printOn:
            print('failed to find good EMu Pair', cat)
        return []
    ...
    if abs(entry.Electron_dxy[i]) < et['ele_dxy']:
        if abs(entry.Electron_dz[i]) < et['ele_dz']:
            if et['ele_ID'] and entry.Electron_mvaFall17V2noIso_WP90[i]:
                if (entry.Electron_lostHits[i]) < et['ele_lostHits']:
                        ...
                        if et['ele_iso_f'] and entry.Electron_pfRelIso03_all[i] < et['ele_iso']:
                            if entry.Electron_pt[i] > et['ele_pt']:
                                if abs(entry.Electron_eta[i]) < et['ele_eta']:
                                    ...
                                    if abs(
                                            entry.Electron_dxy[j]) < et['ele_dxy']:
                                        if abs(
                                                entry.Electron_dz[j]) < et['ele_dz']:
                                            if et['ele_ID'] and entry.Electron_mvaFall17V2noIso_WP90[j]:
                                                if (
                                                        entry.Electron_lostHits[j]) < et['ele_lostHits']:
                                                    if et['ele_convVeto'] and entry.Electron_convVeto[j]:
                                                        if et['ele_iso_f'] and entry.Electron_pfRelIso03_all[j] < et['ele_iso']:
                                                            if entry.Electron_pt[j] > et['ele_pt']:
                                                                if abs(
                                                                        entry.Electron_eta[j]) < et['ele_eta']:

                                                                    if printOn:
                                                                        print('checking overlap....', i, j, DRobj(entry.Electron_eta[i], entry.Electron_phi[i], entry.Electron_eta[j], entry.Electron_phi[j]))
                                                                    if checkDR:
                                                                        if DRobj(
                                                                                entry.Electron_eta[i],
                                                                                entry.Electron_phi[i],
                                                                                entry.Electron_eta[j],
                                                                                entry.Electron_phi[j]) < et['ll_DR']:
                                                                            overlapEl = True
                                                                    else:
                                                                        overlapEl = True
    return overlapEl
...
def getETauPairs(entry, cat='et', pairList=[], printOn=False, signC=0):
    ...
    if printOn:
        print((
            "Entering getETauPairs() nElectron={0:d} nTau={1:d}".format(
                entry.nElectron,
                entry.nTau)))
    if entry.nElectron < 1 or entry.nTau < 1:
        return []
    if cat == 'eeet' and entry.nElectron < 3:
        return []

    eTauPairs = []
    leptOverlap = False
    et = selections['et']  # selections for H->tau(ele)+tau(h)
    if (cat == 'eeet' and entry.nElectron > 3) or (
            cat == 'mmet' and entry.nElectron > 1):
        for i in range(entry.nElectron):
            # if entry.Electron_pt[i]==pairList[0].Pt() or
            # entry.Electron_pt[i]==pairList[1].Pt() : continue # make sure
            # that you don't consider the findZ leptons
            if DRobj(
                    entry.Electron_eta[i],
                    entry.Electron_phi[i],
                    pairList[0].Eta(),
            ...
            for j in range(i + 1, entry.nElectron):
                # if entry.Electron_pt[j]==pairList[0].Pt() or
                # entry.Electron_pt[j]==pairList[1].Pt() : continue# make sure
                # that you don't consider the findZ leptons
                if DRobj(
                        entry.Electron_eta[j],
                        entry.Electron_phi[j],
                        pairList[0].Eta(),
                        pairList[0].Phi()) < 0.1 or DRobj(
                        entry.Electron_eta[j],
                        entry.Electron_phi[j],
                        ...
                        pairList[1].Phi()) < 0.1:
                    continue
                if checkOverlapElectron(entry, i, j):
                    return []
    ...
    for i in range(entry.nElectron):
        ...
        if abs(entry.Electron_dxy[i]) > et['ele_dxy']:
            if printOn:
                print(("    failed dxy={0:f}".format(entry.Electron_dxy[i])))
            continue
        if abs(entry.Electron_dz[i]) > et['ele_dz']:
            if printOn:
                print(("    failed dz={0:f}".format(entry.Electron_dz[i])))
            continue
        if et['ele_ID']:
            if not entry.Electron_mvaFall17V2noIso_WP90[i]:
                if printOn:
                    print((
                        "    failed mva={0}".format(
                            entry.Electron_mvaFall17V2noIso_WP90[i])))
                continue
        if (entry.Electron_lostHits[i]) > et['ele_lostHits']:
            if printOn:
                print((
                    "    failed losthits={0:s}".format(
                        entry.Electron_lostHits[i])))
            continue
        if et['ele_convVeto']:
            if not entry.Electron_convVeto[i]:
                if printOn:
                    print((
                        "    failed convVeto={0}".format(
                            entry.Electron_convVeto[i])))
                continue
        if et['ele_iso_f']:
            if entry.Electron_pfRelIso03_all[i] > et['ele_iso']:
                if printOn:
                    print((
                        "    failed convVeto={0:f}".format(
                            entry.Electron_pfRelIso03_all[i])))
                continue

        if entry.Electron_pt[i] < et['ele_pt']:
            if printOn:
                print(("    failed pt={0:f}".format(entry.Electron_pt[i])))
            continue

        ele_eta, ele_phi = entry.Electron_eta[i], entry.Electron_phi[i]
        if abs(ele_eta) > et['ele_eta']:
            if printOn:
                print(("    failed eta={0:f}".format(entry.Electron_eta[i])))
            continue
        ...
        if DR0 < et['ll_DR'] or DR1 < et['ll_DR']:
            if printOn:
                print(("    failed ltDR DR0={0:f} DR1={1:f}".format(DR0, DR1)))
            continue

        chargeEl = entry.Electron_charge[i]
        if signC != 0 and entry.Electron_charge[i] == signC:
            continue
        ...
        for j in range(entry.nTau):
            ...
            if printOn:
                print(("    tau={0:d}".format(j)))
            if entry.Tau_pt[j] < et['tau_pt']:
                if printOn:
                    print(("        failed pt={0:f}".format(entry.Tau_pt[j])))
                continue
            if abs(entry.Tau_eta[j]) > et['tau_eta']:
                if printOn:
                    print(("        failed eta={0:f}".format(entry.Tau_eta[j])))
                continue

            if et['tau_ID'] and not entry.Tau_idDecayModeNewDMs[j]:
                if printOn:
                    print((
                        "        failed idDecayMode={0}".format(
                            entry.idDecayModeNewDMs[j])))
                continue
            if et['tau_decayMode'] and (
                    entry.Tau_decayMode[j] == 5 or entry.Tau_decayMode[j] == 6):
                if printOn:
                    print((
                        "        failed DecayMode={0}".format(
                            entry.Tau_decayMode[j])))
                continue
            if abs(entry.Tau_dz[j]) > et['tau_dz']:
                if printOn:
                    print(("        failed dz={0:f}".format(entry.Tau_dz[j])))
                continue
            if abs(entry.Tau_charge[j]) != 1:
                if printOn:
                    print((
                        "        failed tauCharge={0:d}".format(
                            entry.Tau_charge[j])))
                continue
            ...
            '''
	    if (entry.Tau_idAntiMu[j]) <= et['tau_antiMu']: continue
            if (entry.Tau_idAntiEle[j]) <= et['tau_antiEle']: continue
            if cat == 'eeet':
                if (entry.Tau_idAntiEle[j]) < et['tau_eeet_antiEle']: continue
	    '''

            '''
            if not (entry.Tau_idDeepTau2017v2p1VSjet[j]) & 16 > 0 : continue
	    if not (entry.Tau_idDeepTau2017v2p1VSe[j]) & 32 > 0 : continue
            if not (entry.Tau_idDeepTau2017v2p1VSmu[j]) & 1 > 0 : continue
            '''

            if et['tau_vJet'] > 0 and not (
                    entry.Tau_idDeepTau2017v2p1VSjet[j]) & et['tau_vJet'] > 0:
                if printOn:
                    print(("        fail DeepTau vs. Jet={0:d}".format(
                        (entry.Tau_idDeepTau2017v2p1VSjet[j]))))
                continue
            if et['tau_vEle'] > 0 and not (
                    entry.Tau_idDeepTau2017v2p1VSe[j]) & et['tau_vEle'] > 0:
                if printOn:
                    print(("        fail DeepTau vs. ele={0:d}".format(
                        (entry.Tau_idDeepTau2017v2p1VSe[j]))))
                continue
            if et['tau_vMu'] > 0 and not (
                    entry.Tau_idDeepTau2017v2p1VSmu[j]) & et['tau_vMu'] > 0:
                if printOn:
                    print(("        fail DeepTau vs.  mu={0:d}".format(
                        (entry.Tau_idDeepTau2017v2p1VSmu[j]))))
                continue
            ...
            tau_eta, tau_phi = entry.Tau_eta[j], entry.Tau_phi[j]
            dPhi = min(abs(tau_phi - ele_phi), 2. *
                       pi - abs(tau_phi - ele_phi))
            DR = sqrt(dPhi**2 + (tau_eta - ele_eta)**2)
            if DR < et['tt_DR']:
                if printOn:
                    print(("        failed ttDR={0:f}".format(DR)))
...
def compareETauPairvspT(entry, tauPairList, printOn=False):
    ...
    for i in range(0, len(tauPairList)):
        ...
        j1, j2 = tauPairList[i][0], tauPairList[i][1]
        if printOn:
            print('appending now', entry.Electron_pt[j1] + entry.Tau_pt[j2], j1, j2, tauPairList[i])
...
def getBestETauPair(entry, cat, pairList=[], printOn=False, signC=0):
    ...
    if printOn:
        print("Entering getBestETauPair")
    # form all possible pairs that satisfy DR requirement
    tauPairList = getETauPairs(
        entry,
        cat=cat,
        pairList=pairList,
        printOn=printOn,
    ...
    if len(tauPairList) == 0:
        if printOn:
            print('failed to find good ETau Pair', cat)
        return []
...
def getEEPairs(entry, cat='ee', pairList=[], printOn=False):
    ...
    if printOn:
        print(("Entering getEEPairs(): nElectron={0:d}".format(entry.nElectron)))

    # need a sufficient number of leptons
    if entry.nElectron < 2:
        return []
    if cat == 'eeee' and entry.nElectron < 4:
        return []
    ...
    for i in range(entry.nElectron):
                ...
                if printOn:
                    print((
                        "\t failed mva={0}".format(
                            entry.Electron_mvaFall17V2noIso_WP90[i])))
                continue
        if (entry.Electron_lostHits[i]) > ee['ele_lostHits']:
            if printOn:
                print((
                    "\t failed losthits={0}".format(
                        entry.Electron_lostHits[i])))
            continue
        if ee['ele_convVeto']:
            if not entry.Electron_convVeto[i]:
        ...
        if DR0 < ee['ll_DR'] or DR1 < ee['ll_DR']:
            if printOn:
                print((
                    "\t failed DR(l_i,e) check: DR0={0:f} DR1={1:f}".format(
                        DR0, DR1)))
            continue
    ...
    for i in selected_elecs:
        for j in selected_elecs:
            ...
            if (j <= i):
                continue
            if printOn:
                print(("\t considering (i,j)=({0:d}, {1:d})".format(i, j)))
            ...
            if DR < ee['tt_DR']:
                if printOn:
                    print(("\t failed eeDR={0:f}".format(DR)))
                continue
...
def compareEEPairs(entry, pair1, pair2):
    ...
    if entry.Electron_pfRelIso03_all[i1] == entry.Electron_pfRelIso03_all[i2]:
        if entry.Electron_pfRelIso03_all[j1] == entry.Electron_pfRelIso03_all[j2]:
            if (entry.Electron_pt[i2] + entry.Electron_pt[j2]
                ) > (entry.Electron_pt[i1] + entry.Electron_pt[j1]):
                return True
...
def getMuMuPairs(entry, cat='mm', pairList=[], printOn=False):
    ...
    if entry.nMuon < 2:
        if printOn:
            print((
                "Entering getMuMuPairs, failing nMuon={0:d}".format(
                    entry.nMuon)))
        return []
    if cat == 'mmmm' and entry.nMuon < 4:
        return []

    if printOn:
        print(("Entering tauFun.getMuMuPairs() nMuon={0:d}".format(entry.nMuon)))
    ...
    for i in range(entry.nMuon):
        ...
        if DR0 < mm['ll_DR'] or DR1 < mm['ll_DR']:
            if printOn:
                print(("\t fail muon DR  DR0={0:f} DR1={1:f}".format(DR0, DR1)))
            continue
        if printOn:
            print(("\t Good muon i={0:d}".format(i)))
    ...
    for i in selected_muons:
        for j in selected_muons:
            ...
            if DR < mm['mt_DR']:
                if printOn:
                    print(("\t fail mmDR DR={0:f}".format(DR)))
                continue
...
def compareMuMuPairs(entry, pair1, pair2):
    ...
    if entry.Muon_pfRelIso04_all[i1] == entry.Muon_pfRelIso04_all[i2]:
        if entry.Muon_pfRelIso04_all[j1] == entry.Muon_pfRelIso04_all[j2]:
            if (entry.Muon_pt[i2] + entry.Muon_pt[j2]
                ) > (entry.Muon_pt[i1] + entry.Muon_pt[j1]):
                return True
...
def getBestMuMuPair(entry, cat='mm', pairList=[], printOn=False):
    ...
    # form all possible pairs that satisfy DR requirement
    if printOn:
        print("Entering getBestMuMuPair()")
...
def goodPhoton(entry, j):
    ...
    if entry.Photon_pt[j] < gj['photon_pt']:
        return False
    if abs(entry.Photon_eta[j]) > gj['photon_eta']:
        return False
    # if entry.Photon_cutBased[j] !=  gj['photon_cutbased']: return False
    if entry.Photon_cutBased[j] < 2 and entry.Photon_mvaID_WP80[j] < 1 and entry.Photon_mvaID_WP90[j] < 1:
        return False
    # if entry.Photon_mvaID_WP80[j] < 0.5 : return False
    if entry.Photon_r9[j] < 0.9:
        return False
    if entry.Photon_electronVeto[j] == 0:
        return False
    if entry.Photon_pixelSeed[j] == 1:
        return False
...
def goodMuonWjets(entry, j):
    ...
    if mm['mu_iso_f'] and entry.Muon_pfRelIso04_all[j] > mm['mu_iso']:
        return False
    if mm['mu_ID']:
        # if not (entry.Muon_tightId[j]) or (entry.Muon_pfIsoId[j]) < 4:
        # return False
    ...
    if abs(entry.Muon_dz[j]) > mm['mu_dz']:
        return False
    if mm['mu_type']:
        # if not (entry.Muon_isGlobal[j] or entry.Muon_isTracker[j]) : return
        # False
        if not (entry.Muon_isGlobal[j]):
            return False
    return True
...
def makeGoodPhotonList(entry):
    ...
    for i in range(entry.nPhoton):
        if goodPhoton(entry, i):
            goodPhotonList.append(i)
...
def goodElectron(entry, j):
    ...
    if abs(entry.Electron_eta[j]) > ee['ele_eta']:
        return False
    if abs(entry.Electron_eta[j]) > 1.44 and abs(entry.Electron_eta[j]) < 1.57:
        return False
    if abs(entry.Electron_dxy[j]) > ee['ele_dxy']:
        return False
    if abs(entry.Electron_dz[j]) > ee['ele_dz']:
        return False
    if (entry.Electron_lostHits[j]) > ee['ele_lostHits']:
        return False
...
def vetoElectron(entry, j):
    ...
    # (entry.Electron_lostHits[j]) < ee['ele_lostHits'] and \
    # entry.Electron_convVeto[j] and \

    # entry.Electron_cutBased[j] == ee['ele_cut'] : return True

    # entry.Electron_mvaFall17V2noIso_WP90[j] and\
    # if entry.luminosityBlock==159 and entry.run==320853 and
    # entry.event==197659973: print entry.nElectron, j, 'pt: ',
    # entry.Electron_pt[j], 'eta :', abs(entry.Electron_eta[j]), 'hits: ',
    # (entry.Electron_lostHits[j]), 'iso :',
    # entry.Electron_pfRelIso03_all[j], 'veto: ',entry.Electron_convVeto[j],
    # 'iD: ', entry.Electron_mvaFall17V2noIso_WP90[j], entry.event

    # if (entry.Electron_lostHits[j]) > ee['ele_lostHits']: return False
...
def makevetoElectronList(entry):
    vetoElectronList = []
    for i in range(entry.nElectron):
        if vetoElectron(entry, i):
            vetoElectronList.append(i)
...
def goodElectronDCH(entry, j):
    ...
    if ee['ele_convVeto']:
        if not entry.Electron_convVeto[j]:
            return False
    if ee['ele_ID']:
        if not entry.Electron_cutBased_HEEP[j]:
            return False
...
def goodMuonDCH(entry, j):
    ...
    # if mm['mu_iso_f'] and entry.Muon_pfRelIso04_all[j] >  mm['mu_iso']:
    # return False
    if mm['mu_iso_f'] and entry.Muon_tkRelIso[j] > mm['mu_iso']:
        return False
    if mm['mu_ID']:
        # if not (entry.Muon_mediumId[j] or entry.Muon_tightId[j]): return
        # False
        if not (entry.Muon_mediumId[j]):
            return False
    # if mm['mu_ID'] and not entry.Muon_looseId[j] : return False
    if abs(entry.Muon_dxy[j]) > mm['mu_dxy']:
        return False
...
def goodMuonExtraLepton(entry, j):
    ...
    if mm['mu_iso_f'] and entry.Muon_pfRelIso04_all[j] > mm['mu_iso']:
        return False
    if mm['mu_ID'] and not (entry.Muon_mediumId[j] or entry.Muon_tightId[j]):
        return False
    if abs(entry.Muon_dxy[j]) > mm['mu_dxy']:
        return False
    if abs(entry.Muon_dz[j]) > mm['mu_dz']:
...
def makeGoodMuonListExtraLepton(entry, listMu):
    goodExtraMuonList = []
    for i in range(entry.nMuon):
        if i not in listMu and goodMuonExtraLepton(entry, i):
            goodExtraMuonList.append(i)
    #print("In tauFun.makeGoodMuonList = {0:s}".format(str(goodMuonList)))
...
def makeGoodElectronListExtraLepton(entry, listEl):
    ...
    for i in range(entry.nElectron):
        if i not in listEl and goodElectronExtraLepton(entry, i):
            goodExtraElectronList.append(i)
    return goodExtraElectronList


def eliminateCloseTauAndLepton(
        entry,
        goodElectronList,
        goodMuonList,
        goodTauList):
    ...
    for tau1 in goodTauList:
        for tau2 in goodTauList:
            ...
            if sqrt(dEta * dEta + dPhi * dPhi) > 0.3:
                continue
            if not (
                    tau1 in badTau) and entry.Tau_pt[tau1] < entry.Tau_pt[tau2]:
                badTau.append(tau1)
            if not (
                    tau2 in badTau) and entry.Tau_pt[tau1] > entry.Tau_pt[tau2]:
                badTau.append(tau2)
        ...
        for mu2 in goodMuonList:
            ...
            if sqrt(dEta * dEta + dPhi * dPhi) > 0.3:
                continue
            if not (
                    tau1 in badTau) and entry.Tau_pt[tau1] < entry.Muon_pt[mu2]:
                badTau.append(tau1)
            if not (
                    mu2 in badMuon) and entry.Tau_pt[tau1] > entry.Muon_pt[mu2]:
                badMuon.append(mu2)
        ...
        for e2 in goodElectronList:
            ...
            if sqrt(dEta * dEta + dPhi * dPhi) > 0.3:
                continue
            if not (tau1 in badTau):
                badTau.append(tau1)
            if not (e2 in badElectron):
                badElectron.append(e2)
            if not (
                    tau1 in badTau) and entry.Tau_pt[tau1] < entry.Electron_pt[e2]:
                badTau.append(tau1)
            if not (
                    e2 in badElectron) and entry.Tau_pt[tau1] > entry.Electron_pt[e2]:
                badElectron.append(e2)
    ...
    for e1 in goodElectronList:
        for e2 in goodElectronList:
            ...
            if sqrt(dEta * dEta + dPhi * dPhi) > 0.3:
                continue
            if not (
                    e1 in badElectron) and entry.Electron_pt[e1] < entry.Electron_pt[e2]:
                badElectron.append(e1)
            if not (
                    e2 in badElectron) and entry.Electron_pt[e1] > entry.Electron_pt[e2]:
                badElectron.append(e2)
    ...
    for mu1 in goodMuonList:
        for mu2 in goodMuonList:
            ...
            if sqrt(dEta * dEta + dPhi * dPhi) > 0.3:
                continue
            if not (
                    mu1 in badMuon) and entry.Muon_pt[mu1] < entry.Muon_pt[mu2]:
                badMuon.append(mu1)
            if not (
                    mu2 in badMuon) and entry.Muon_pt[mu1] > entry.Muon_pt[mu2]:
                badMuon.append(mu2)
        ...
        for e2 in goodElectronList:
            ...
            if sqrt(dEta * dEta + dPhi * dPhi) > 0.3:
                continue
            if not (
                    mu1 in badMuon) and entry.Muon_pt[mu1] < entry.Electron_pt[e2]:
                badMuon.append(mu1)
            if not (
                    e2 in badElectron) and entry.Muon_pt[mu1] > entry.Electron_pt[e2]:
                badElectron.append(e2)

    for bade in badElectron:
        goodElectronList.remove(bade)
    for badmu in badMuon:
        goodMuonList.remove(badmu)
    for badtau in badTau:
        goodTauList.remove(badtau)
...
def findETrigger(goodElectronList, entry, era):
    ...
    if nElectron > 1:
        if era == '2016' and not entry.HLT_Ele27_WPTight_Gsf:
            return EltrigList
        if era == '2017' and not entry.HLT_Ele35_WPTight_Gsf:
            return EltrigList
        for i in range(nElectron):

            ii = goodElectronList[i]
            if era == '2016' and entry.Electron_pt[ii] < 29:
                continue
            if era == '2017' and entry.Electron_pt[ii] < 37:
                continue
            ...
            for iobj in range(0, entry.nTrigObj):
                dR = DRobj(
                    entry.Electron_eta[ii],
                    entry.Electron_phi[ii],
                    entry.TrigObj_eta[iobj],
                    entry.TrigObj_phi[iobj])
                ...
                # that corresponds 0 WPTight
                if entry.TrigObj_filterBits[iobj] & 2 and dR < 0.5:
                    EltrigList.append(ii)
                    # print "======================= iobj", iobj,
                    # "entry.Trig",entry.TrigObj_id[iobj], "Bits",
                    # entry.TrigObj_filterBits[iobj]," dR", dR,
                    # "electron",i,"ii",ii,entry.TrigObj_id[iobj]
...
def findMuTrigger(goodMuonList, entry, era):
    ...
    if nMuon > 1:
        if era == '2016' and not entry.HLT_IsoMu24:
            return MutrigList
        if era == '2017' and not entry.HLT_IsoMu27:
            return MutrigList
        for i in range(nMuon):

            ii = goodMuonList[i]
            if era == '2016' and entry.Muon_pt[ii] < 26:
                continue
            if era == '2017' and entry.Muon_pt[ii] < 29:
                continue
            ...
            for iobj in range(0, entry.nTrigObj):
                ...
                # entry.TrigObj_eta[iobj], entry.TrigObj_phi[iobj], dR,
                # entry.TrigObj_filterBits[iobj]))
                # that corresponds to Muon Trigger
                if entry.TrigObj_filterBits[iobj] & 8 or entry.TrigObj_filterBits[iobj] & 2 and dR < 0.5:
                    MutrigList.append(ii)
                # print "======================= and === iobj", iobj,
                # entry.TrigObj_id[iobj], "Bits",
                # entry.TrigObj_filterBits[iobj]," dR", dR, "electron",i

    return MutrigList


def ComparepT(El, Mu, entry):
    if entry.Electron_pt[El] > entry.Muon_pt[Mu]:
        return True
    else:
        return False
        ...
        for i in range(nElectron):
            ...
            if entry.Electron_charge[ii] > 0:
                for j in range(i + 1, nElectron):
                    ...
                    # print 'going in tauFun masses', goodElectronList,
                    # nElectron, entry.event, entry.luminosityBlock, entry.run,
                    # 'for', jj, ii, entry.Electron_charge[ii],
                    # entry.Electron_charge[jj]
                    if entry.Electron_charge[ii] == entry.Electron_charge[jj]:
                        ...
                        mass = cand.M()
                        # print 'going in tauFun masses', goodElectronList,
                        # nElectron, entry.event, entry.luminosityBlock,
                        # entry.run, bestDiff, 'is abs(mass-mZ > bestDiff',
                        # abs(mass-mZ), bestDiff, 'for', jj, ii
                        bestDiff = abs(mass)
                        # print 'masses', bestDiff, 'mass', mass, 'q_1',
                        # entry.Electron_charge[jj], 'q_2',
                        # entry.Electron_charge[ii], entry.event,
                        # entry.luminosityBlock, entry.run, 'elect', jj, ii,
                        # goodElectronList, entry.Electron_pt[ii],
                        # entry.Electron_pt[jj]
        ...
        for i in range(nMuon):
            ...
            if entry.Muon_charge[ii] > 0:
                for j in range(i + 1, nMuon):
                    jj = goodMuonList[j]
                    # print 'going in tauFun masses', goodMuonList, nMuon,
                    # entry.event, entry.luminosityBlock, entry.run, 'for', jj,
                    # ii, entry.Muon_charge[ii],  entry.Muon_charge[jj]
                    if entry.Muon_charge[ii] == entry.Muon_charge[jj]:
                        ...
                        #bestDiff = abs(mass)
                        # print 'masses', bestDiff, 'mass', mass, 'q_1',
                        # entry.Muon_charge[jj], 'q_2', entry.Muon_charge[ii],
                        # entry.event, entry.luminosityBlock, entry.run,
                        # 'muons', jj, ii, goodMuonList, entry.Muon_pt[ii],
                        # entry.Muon_pt[jj]
...
def findW(goodElectronList, goodMuonList, entry):
    ...
    nElectron = len(goodElectronList)
    # print 'going in tauFun', goodElectronList, nElectron, entry.event,
    # entry.luminosityBlock, entry.run, bestDiff
    if nElectron > 1:
        for i in range(nElectron):
            ii = goodElectronList[i]
            e1 = TLorentzVector()
            ...
            for j in range(i + 1, nElectron):
                jj = goodElectronList[j]
                # print 'going in tauFun masses', goodElectronList, nElectron,
                # entry.event, entry.luminosityBlock, entry.run, 'for', jj, ii,
                ...
                if entry.Electron_charge[ii] != entry.Electron_charge[jj]:
                    ...
                    # print 'going in tauFun masses', goodElectronList,
                    # nElectron, entry.event, entry.luminosityBlock, entry.run,
                    # bestDiff, 'is abs(mass-mZ > bestDiff', abs(mass-mZ),
                    # bestDiff, 'for', jj, ii
                    if abs(mass - mZ) < bestDiff:
                        bestDiff = abs(mass - mZ)
                        # print 'masses', bestDiff, mass,
                        # entry.Electron_charge[jj], entry.Electron_charge[ii],
                        # entry.event, entry.luminosityBlock, entry.run,
                        # 'elect', jj, ii, goodElectronList
                        if entry.Electron_charge[ii] > 0.:
                            pairList = [e1, e2]
                            selpair = [ii, jj]
                        else:
    ...
    # first particle of pair is positive
    # print 'returning', selpair,  'is muon', nMuon, goodMuonList, 'isEl',
    # nElectron, goodElectronList, entry.event, entry.luminosityBlock,
    # entry.run
