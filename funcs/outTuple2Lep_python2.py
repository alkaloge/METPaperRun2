...
class outTuple2Lep():
    ...
    def Fill2Lep(
        ...
        if '2016' in era:
            SaveEventMu = cat == 'mm' and (
                self.isGlobal_1[0] > 0 or self.isTracker_1[0] > 0) and self.pt_1[0] > 10 and self.mediumId_1[0] > 0 and fabs(
                self.eta_1[0]) < 2.4 and fabs(
                self.dZ_1[0]) < 0.2 and fabs(
                self.d0_1[0]) < 0.045 and self.iso_1[0] < 0.2 and (
                    self.isGlobal_2[0] > 0 or self.isTracker_2[0] > 0) and self.pt_2[0] > 10 and self.mediumId_2[0] > 0 and fabs(
                        self.eta_2[0]) < 2.4 and fabs(
                            self.dZ_2[0]) < 0.2 and fabs(
                                self.d0_2[0]) < 0.045 and self.iso_2[0] < 0.2 and self.isTrig_1[0] != 0
            SaveEventEl = cat == 'ee' and self.pt_1[0] > 10 and self.Electron_mvaFall17V2Iso_WP90_1[0] > 0 and fabs(
                self.eta_1[0]) < 2.1 and not (
                fabs(
                    self.eta_1[0]) > 1.4442 and fabs(
                    self.eta_1[0]) < 1.5660) and fabs(
                    self.dZ_1[0]) < 0.2 and fabs(
                        self.d0_1[0]) < 0.045 and self.iso_1[0] < 0.15 and self.pt_2[0] > 10 and self.Electron_mvaFall17V2Iso_WP90_2[0] > 0 and fabs(
                            self.eta_2[0]) < 2.1 and not (
                                fabs(
                                    self.eta_2[0]) > 1.4442 and fabs(
                                        self.eta_2[0]) < 1.5660) and fabs(
                                            self.dZ_2[0]) < 0.2 and fabs(
                                                self.d0_2[0]) < 0.045 and self.iso_2[0] < 0.15 and self.isTrig_1[0] != 0
        ...
        if SaveEventMu or SaveEventEl:
            ...
            if isMC:
                        ...
                        if leadL.Pt() > 26:
                            muontrig1 = self.evaluator["NUM_IsoMu24_DEN_CutBasedIdTight_and_PFIsoTight"].evaluate(
                                ...
                                fabs(
                                    leadL.Eta()),
                                leadL.Pt(),
                                "sf")  # for 2018 the json Veto only 24 SF..but we have used the HLT27
                            self.TrigSF1_up[0] = self.evaluator["NUM_IsoMu24_DEN_CutBasedIdTight_and_PFIsoTight"].evaluate(
                                "{0:s}_UL".format(
                                    str(era)),
                                fabs(
                                    leadL.Eta()),
                                leadL.Pt(),
                                "systup")  # for 2018 the json Veto only 24 SF..but we have used the HLT27
                            self.TrigSF1_down[0] = self.evaluator["NUM_IsoMu24_DEN_CutBasedIdTight_and_PFIsoTight"].evaluate(
                                ...
                                fabs(
                                    leadL.Eta()),
                                leadL.Pt(),
                                "systdown")  # for 2018 the json Veto only 24 SF..but we have used the HLT27
                        if subL.Pt() > 26:
                            muontrig2 = self.evaluator["NUM_IsoMu24_DEN_CutBasedIdTight_and_PFIsoTight"].evaluate(
                                ...
                                fabs(
                                    subL.Eta()),
                                subL.Pt(),
                                "sf")  # fora 2018 the json Veto only 24 SF..but we have used the HLT27
                ...
                if channel_ll == 'ee':
                    ...
                    eff_trig_d_1 = self.sf_EleTrig.get_EfficiencyData(
                        leadL.Pt, leadL.Eta())
                    eff_trig_mc_1 = self.sf_EleTrig.get_EfficiencyMC(
                        leadL.Pt, leadL.Eta())
                    eff_trig_mc_2 = self.sf_EleTrig.get_EfficiencyMC(
                        subL.Pt, subL.Eta())
                    eff_trig_d_2 = self.sf_EleTrig.get_EfficiencyData(
                        subL.Pt, subL.Eta())
