...
def printTriggerObjects(e):
    ...
    for i in range(e.nTrigObj):
        print(
            (
                '{0:3d}\t{1:5.1f}\t{2:5.2f}\t{3:5.2f}\t{4:5d}0x\t{5:04x}'.format(
                    i,
                    e.TrigObj_pt[i],
                    e.TrigObj_phi[i],
                    e.TrigObj_eta[i],
                    e.TrigObj_id[i],
                    e.TrigObj_filterBits[i])))
    return
...
def genMatchTau(entry, jt, decayMode=''):
    ...
    if decayMode == 'had':
        for i in range(entry.nGenVisTau):
            dPhi = min(abs(entry.GenVisTau_phi[i] -
                           entry.Tau_phi[jt]), 2.0 *
                       pi -
                       abs(entry.GenVisTau_phi[i] -
                           entry.Tau_phi[jt]))
    ...
    if decayMode == 'lep':
        for i in range(entry.nGenPart):
            if abs(entry.GenPart_pdgId[i]) != 15:
                continue
            dPhi = min(abs(entry.GenPart_phi[i] -
                           entry.Tau_phi[jt]), 2.0 *
                       pi -
                       abs(entry.GenPart_phi[i] -
                           entry.Tau_phi[jt]))
...
def checkMETFlags(entry, year, isMC=False, proc="UL"):
    ...
    if proc == 'EOY':
        if '2016' in str(year) and (entry.Flag_goodVertices == False or entry.Flag_globalSuperTightHalo2016Filter == False or entry.Flag_HBHENoiseFilter ==
                                    False or entry.Flag_HBHENoiseIsoFilter == False or entry.Flag_EcalDeadCellTriggerPrimitiveFilter == False or entry.Flag_BadPFMuonFilter == False):
            METfilter = True

        if '2018' in str(year) and (entry.Flag_goodVertices == False or entry.Flag_globalSuperTightHalo2016Filter == False or entry.Flag_HBHENoiseFilter == False or entry.Flag_HBHENoiseIsoFilter ==
                                    False or entry.Flag_EcalDeadCellTriggerPrimitiveFilter == False or entry.Flag_BadPFMuonFilter == False or entry.Flag_eeBadScFilter == False or entry.Flag_ecalBadCalibFilter == False):
            METfilter = True

        #if not isMC and entry.Flag_eeBadScFilter == False : METfilter = True
    if proc == 'UL':
        if '2016' in str(year) and (entry.Flag_goodVertices == False or entry.Flag_globalSuperTightHalo2016Filter == False or entry.Flag_HBHENoiseFilter ==
                                    False or entry.Flag_HBHENoiseIsoFilter == False or entry.Flag_EcalDeadCellTriggerPrimitiveFilter == False or entry.Flag_BadPFMuonFilter == False or entry.Flag_eeBadScFilter == False):
            METfilter = True

        if '2016' not in str(year) and (entry.Flag_goodVertices == False or entry.Flag_globalSuperTightHalo2016Filter == False or entry.Flag_HBHENoiseFilter == False or entry.Flag_HBHENoiseIsoFilter ==
                                        False or entry.Flag_EcalDeadCellTriggerPrimitiveFilter == False or entry.Flag_BadPFMuonFilter == False or entry.Flag_eeBadScFilter == False or entry.Flag_ecalBadCalibFilter == False):
            METfilter = True

    # print metdz, METfilter, 'filters', entry.Flag_goodVertices,
    # entry.Flag_globalSuperTightHalo2016Filter, entry.Flag_HBHENoiseFilter,
    # entry.Flag_HBHENoiseIsoFilter,entry.Flag_EcalDeadCellTriggerPrimitiveFilter,
    # entry.Flag_BadPFMuonFilter,
    # entry.Flag_eeBadScFilter,entry.Flag_ecalBadCalibFilter,
...
def printEvent(entry):
    ...
    print(("** Run={0:d} LS={1:d} Event={2:d} MET={3:.1f}".format(entry.run,
          entry.luminosityBlock, entry.event, entry.MET_pt)))
    if entry.nMuon > 0:
        print("Muons\n # Q    Pt   Eta   Phi   Iso  tkRelIso Medium Tight Soft    dxy     dz   MC     dR     Pt   eta   phi, genMatch")
        for j in range(entry.nMuon):
            muSign = '+'
            if entry.Muon_charge[j] < 0:
                muSign = '-'
            try:
                print(("{0:2d} {1:2s}{2:5.1f}{3:6.2f}{4:6.2f}{12:6.2f}{5:7.3f} {6:5s} {7:5s} {8:5s}{9:7.3f}{10:7.3f}{11:s}".format(
                    ...
                    j, muSign, entry.Muon_pt[j], entry.Muon_eta[j], entry.Muon_phi[j], entry.Muon_tkRelIso[j], str(
                        entry.Muon_mediumId[j]), str(entry.Muon_tightId[j]),
                    str(entry.Muon_softId[j]
                        ), entry.Muon_dxy[j], entry.Muon_dz[j],
                    getMCmatchString(entry.Muon_eta[j], entry.Muon_phi[j], entry), entry.Muon_tkRelIso[j])), ord(entry.Muon_genPartFlav[j]))
            except AttributeError:
                print(
                    (
                        "{0:2d} {1:2s}{2:5.1f}{3:6.2f}{4:6.2f}{5:7.3f} {6:5s} {7:5s} {8:5s}".format(
                            j, muSign, entry.Muon_pt[j], entry.Muon_eta[j], entry.Muon_phi[j], entry.Muon_pfRelIso04_all[j], str(
                                entry.Muon_mediumId[j]), str(
                                entry.Muon_tightId[j]), str(
                                entry.Muon_softId[j]), entry.Muon_dxy[j], entry.Muon_dz[j])))
    ...
    if entry.nElectron > 0:
        ...
        for j in range(entry.nElectron):
            eSign = '+'
            if entry.Electron_charge[j] < 0:
                eSign = '-'
            try:
                print(("{0:2d} {1:2s}{2:5.1f}{3:6.2f}{4:6.2f}{5:7.3f}{6:6d}{7:5d}{8:7.3f} {9} {10:7.3f}{11:7.3f}{12:s} {13:f}".format(j,
                                                                                                                                      eSign,
                                                                                                                                      entry.Electron_pt[j],
                                                                                                                                      entry.Electron_eta[j],
                                                                                                                                      entry.Electron_phi[j],
                                                                                                                                      entry.Electron_miniPFRelIso_all[j],
                                                                                                                                      entry.Electron_cutBased[j],
                                                                                                                                      ord(entry.Electron_lostHits[j]),
                                                                                                                                      entry.Electron_mvaFall17V2noIso[j],
                                                                                                                                      entry.Electron_mvaFall17V2noIso_WP90[j],
                                                                                                                                      entry.Electron_dxy[j],
                                                                                                                                      entry.Electron_dz[j],
                                                                                                                                      getMCmatchString(entry.Electron_eta[j],
                                                                                                                                                       entry.Electron_phi[j],
                                                                                                                                                       entry))),
                      ord(entry.Electron_genPartFlav[j]),
                      entry.Electron_cutBased[j])
            except AttributeError:
                ...
                # entry.Electron_cutBased[j],  ord(entry.Electron_lostHits[j]),
                # entry.Electron_dxy[j], entry.Electron_dz[j]),
                # ord(entry.Electron_genPartFlav[j]),
                # entry.Electron_cutBased[j])
                print(
                    (
                        "{0:2d} {1:2s} {2:5.1f} {3:6.2f} {4:6.2f} {5:7.3f} {6:7.3f} {7:f} {8:f} ".format(
                            j, eSign, entry.Electron_pt[j], entry.Electron_pfRelIso03_all[j], ord(
                                entry.Electron_lostHits[j]), entry.Electron_dxy[j], entry.Electron_dz[j], ord(
                                entry.Electron_genPartFlav[j]), entry.Electron_cutBased[j])))
    ...
    if entry.nJet > 0:
        ...
        for j in range(entry.nJet):
            print(("{0:2d} {1:5.1f}{2:6.2f}{3:6.2f}{4:6d}{5:8.3f}".format(j,
                                                                          entry.Jet_pt[j],
                                                                          entry.Jet_eta[j],
                                                                          entry.Jet_phi[j],
                                                                          entry.Jet_jetId[j],
                                                                          entry.Jet_btagCSVV2[j])))
        #print("JetList\n     Pt     Eta    Phi ")
        #i = 0
    ...
    if False and entry.nTau > 0:
        ...
        for j in range(entry.nTau):
            try:
                print(
                    (
                        "{0:2d} {1:5.1f}{2:6.2f}{3:6.2f}{4:5d}  {5:5s} {6:5s}{18:4d}{19:4d}{20:4d} {7:6.2f}{8:6.2f}{9:6.2f}{10:6d}{11:6d}{12:6d}  {13:7.3f}{14:7.3f} {15:5d} {16:8.4f} {17:6s}".format(
                            j, entry.Tau_pt[j], entry.Tau_eta[j], entry.Tau_phi[j], entry.Tau_decayMode[j], str(
                                entry.Tau_idDecayMode[j]), str(
                                entry.Tau_idDecayModeNewDMs[j]), entry.Tau_rawIso[j], entry.Tau_chargedIso[j], entry.Tau_neutralIso[j], entry.Tau_jetIdx[j], ord(
                                entry.Tau_idAntiEle[j]), ord(
                                entry.Tau_idAntiMu[j]), entry.Tau_dxy[j], entry.Tau_dz[j], ord(
                                entry.Tau_idMVAoldDM2017v2[j]), entry.Tau_rawMVAoldDM2017v2[j], getMCmatchString(
                                    entry.Tau_eta[j], entry.Tau_phi[j], entry)[
                                        0:6], ord(
                                            entry.Tau_idDeepTau2017v2p1VSjet[j]), ord(
                                                entry.Tau_idDeepTau2017v2p1VSmu[j]), ord(
                                                    entry.Tau_idDeepTau2017v2p1VSe[j]))), ord(
                                                        entry.Tau_genPartFlav[j]))
            except AttributeError:
                print(
                    (
                        "{0:2d} {1:5.1f}{2:6.2f}{3:6.2f}{4:5d}  {5:5s} {6:5s}{18:4d}{19:4d}{20:4d} {7:6.2f}{8:6.2f}{9:6.2f}{10:6d}{11:6d}{12:6d}  {13:7.3f}{14:7.3f} {15:5d} {16:8.4f}".format(
                            j, entry.Tau_pt[j], entry.Tau_eta[j], entry.Tau_phi[j], entry.Tau_decayMode[j], str(
                                entry.Tau_idDecayMode[j]), str(
                                entry.Tau_idDecayModeNewDMs[j]), entry.Tau_rawIso[j], entry.Tau_chargedIso[j], entry.Tau_neutralIso[j], entry.Tau_jetIdx[j], ord(
                                entry.Tau_idAntiEle[j]), ord(
    ...
    if True and entry.nTrigObj > 0:
        ...
        for j in range(entry.nTrigObj):
            ...
            if tID == "   Tau":
                print(
                    (
                        "{0:2d} {1:5.1f}{2:6.2f}{3:6.2f} {4:8s}{5:4X}".format(
                            j,
                            entry.TrigObj_pt[j],
                            entry.TrigObj_eta[j],
                            entry.TrigObj_phi[j],
                            tID,
                            entry.TrigObj_filterBits[j])))
...
def printMC(entry):
    PDG_ID = getPDG_ID()
    print(("\n** MC ** Run={0:d} LS={1:d} Event={2:d} MET={3:.1f}".format(
        entry.run, entry.luminosityBlock, entry.event, entry.MET_pt)))
    try:
        if entry.nGenPart > 0:
            ...
            for j in range(entry.nGenPart):
                ...
                print(
                    (
                        "{0:2d}{1:4d}  {2:6s}{3:6.1f} {4:6d}{5:7.1f}{6:9.2f}{7:6.2f}".format(
                            j,
                            entry.GenPart_status[j],
                            str(pID),
                            entry.GenPart_mass[j],
                            mother,
                            entry.GenPart_pt[j],
                            entry.GenPart_eta[j],
                            entry.GenPart_phi[j])))
...
def hasZmumu(entry):
    ...
    for j in range(entry.nGenPart):
        ...
        if entry.GenPart_pdgId[k] == 23:
            if entry.GenPart_pdgId[j] == 13:
                hasMuM = True
            if entry.GenPart_pdgId[j] == -13:
                hasMuP = True
            if hasMuM and hasMuP:
                return True
    return False
...
def getMCmatchString(eta, phi, entry):
    ...
    for j in range(entry.nGenPart):
        if entry.GenPart_status[j] != 1:
            continue
        deltaR = sqrt(
            (phi - entry.GenPart_phi[j])**2 + (eta - entry.GenPart_eta[j])**2)
    ...
    return " {0:6s}{1:6.3f}{2:6.1f}{3:6.2f}{4:6.2f}".format(
        pID,
        smallestDeltaR,
        entry.GenPart_pt[jBest],
        entry.GenPart_eta[jBest],
        entry.GenPart_phi[jBest])
    if jBest == 999:
        return '*'
    return '**'
...
def findDoubleLeptTrigger(goodLeptonList, entry, flavour, era):
    ...
    if 'ee' in flavour and nLepton > 1:
        try:
            HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ = entry.HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ
        except AttributeError:
        ...
        try:
            HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL = entry.HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL
        except AttributeError:
            HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL = False

        if '2016' in era and not HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ:
        ...
        if entry.Electron_pt[goodLeptonList[0]
                             ] > entry.Electron_pt[goodLeptonList[1]]:
        ...
        if entry.Electron_pt[leadL] < 25 or entry.Electron_pt[subleadL] < 14:
            return LepttrigList, hltList

    # if flavour == 'ee' :print 'pT ', entry.Electron_pt[leadL],
    # entry.Electron_pt[subleadL], leadL, subleadL

    if 'mm' in flavour and nLepton > 1:
        try:
            HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ = entry.HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ
        except AttributeError:
            HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ = False
        try:
            HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ = entry.HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ
        except AttributeError:
            HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ = False
        try:
            HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_Mass8 = entry.HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_Mass8
        except AttributeError:
            HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_Mass8 = False

        if '2016' in era and not HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ and not HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ:
            return LepttrigList, hltList
        if '2016' not in era and not HLT_Mu17_TrkIsoVVL_TkMu8_TrkIsoVVL_DZ_Mass8:
            return LepttrigList, hltList
        ...
        if entry.Muon_pt[leadL] < 19 or entry.Muon_pt[subleadL] < 10:
            return LepttrigList, hltList
    # for electron triggers 1 = CaloIdL_TrackIdL_IsoVL, 2 = 1e (WPTight), 4 =
    # 1e (WPLoose), 8 = OverlapFilter PFTau, 16 = 2e, 32 = 1e-1mu, 64 =
    # 1e-1tau, 128 = 3e, 256 = 2e-1mu, 512 = 1e-2mu, 1024 = 1e
    # (32_L1DoubleEG_AND_L1SingleEGOr), 2048 = 1e (CaloIdVT_GsfTrkIdT), 4096 =
    # 1e (PFJet), 8192 = 1e (Photon175_OR_Photon200) for Electron
    ...
    # for 2016 in particular  "1 = TrkIsoVVL, 2 = Iso, 4 = OverlapFilter
    # PFTau, 8 = IsoTkMu"
    dR = 100.
    dRr = 100.
    i_lead = -1
    ...
    for iobj in range(0, entry.nTrigObj):
        if 'ee' in flavour and abs(entry.TrigObj_id[iobj]) == 11:
            dR = DRobj(
                entry.Electron_eta[leadL],
                entry.Electron_phi[leadL],
            ...
            for iobjj in range(iobj, entry.nTrigObj):
                dRr = DRobj(
                    entry.Electron_eta[subleadL],
                    entry.Electron_phi[subleadL],
                    entry.TrigObj_eta[iobjj],
                    entry.TrigObj_phi[iobjj])
        ...
        if 'mm' in flavour and abs(entry.TrigObj_id[iobj]) == 13:
            ...
            dR = DRobj(
                entry.Muon_eta[leadL],
                entry.Muon_phi[leadL],
                entry.TrigObj_eta[iobj],
                entry.TrigObj_phi[iobj])
            ...
            for iobjj in range(iobj, entry.nTrigObj):
                dRr = DRobj(
                    entry.Muon_eta[subleadL],
                    entry.Muon_phi[subleadL],
                    entry.TrigObj_eta[iobjj],
                    entry.TrigObj_phi[iobjj])
    ...
    # if 'ee' in flavour  : print '=============', dR, dRr, i_trail, i_lead,
    # hltList
    if i_lead != i_trail and i_lead != -1 and i_trail != -1:
...
def findSinglePhotonTrigger(entry, era, printOn=False):

    ps = 1
    if entry.HLT_Photon165_R9Id90_HE10_IsoM:
        ps = 4
    else:
        if entry.HLT_Photon120_R9Id90_HE10_IsoM:
            ps = 8
        else:
            if entry.HLT_Photon90_R9Id90_HE10_IsoM:
                ps = 32
            else:
                if entry.HLT_Photon75_R9Id90_HE10_IsoM:
                    ps = 64
                else:
                    if entry.HLT_Photon50_R9Id90_HE10_IsoM:
                        ps = 256
                    else:
                        ps = 0
    return ps
...
def findSinglePhotonTriggerName(entry, photonindex, era, printOn=False):

    ps = ''
    isFired = 0
    if entry.Photon_pt[photonindex] < 80 and entry.HLT_Photon50_R9Id90_HE10_IsoM:
        ps = "HLT_Photon50_R9Id90_HE10_IsoM"
    else:
        if entry.Photon_pt[photonindex] >= 80 and entry.Photon_pt[photonindex] < 95 and entry.HLT_Photon75_R9Id90_HE10_IsoM:
            ps = "HLT_Photon75_R9Id90_HE10_IsoM"
        else:
            if entry.Photon_pt[photonindex] >= 95 and entry.Photon_pt[photonindex] < 130 and entry.HLT_Photon90_R9Id90_HE10_IsoM:
                ps = "HLT_Photon90_R9Id90_HE10_IsoM"
            else:
                if entry.Photon_pt[photonindex] >= 130 and entry.Photon_pt[photonindex] < 175 and entry.HLT_Photon120_R9Id90_HE10_IsoM:
                    ps = "HLT_Photon120_R9Id90_HE10_IsoM"
                else:
                    if entry.Photon_pt[photonindex] >= 175 and entry.HLT_Photon165_R9Id90_HE10_IsoM:
                        ps = "HLT_Photon165_R9Id90_HE10_IsoM"
                    else:
                        ps = ''
...
def findSinglePhotonTrigger2(entry, photonindex, era, printOn=False):
    ...
    for iobj in range(0, entry.nTrigObj):
        ...
        dR = DRobj(
            entry.Photon_eta[photonindex],
            entry.Photon_phi[photonindex],
            entry.TrigObj_eta[iobj],
            entry.TrigObj_phi[iobj])
        # print 'dR-->',  iobj, dR, 'iD', entry.TrigObj_id[iobj], 'filterBit', entry.TrigObj_filterBits[iobj], 'charge', entry.TrigObj_l1charge[iobj], 'l1pt', entry.TrigObj_l1pt[iobj], 'l2pt', entry.TrigObj_l2pt[iobj], 'pT', entry.TrigObj_pt[iobj], 'g_pt ', entry.Photon_pt[0], 'obj_phi', entry.TrigObj_phi[iobj], 'g_phi', entry.Photon_phi[0],  ps
        # print abs(entry.TrigObj_id[iobj]), dR, entry.TrigObj_filterBits[iobj]
        if abs(entry.TrigObj_id[iobj]) == 22 and dR < 0.5:
            isFired = 1

    return ps, isFired


def findSinglePhotonTriggerYaml(entry, photonindex, year, printOn=False):
    era = str(year)
    ps = 1
    if entry.Photon_pt[photonindex] < 80 and entry.HLT_Photon50_R9Id90_HE10_IsoM:
        ps = GPr.get_value_from_yaml(
            era,
        ...
        if entry.Photon_pt[photonindex] >= 80 and entry.Photon_pt[photonindex] < 95 and entry.HLT_Photon75_R9Id90_HE10_IsoM:
            ...
            if entry.Photon_pt[photonindex] >= 95 and entry.Photon_pt[photonindex] < 130 and entry.HLT_Photon90_R9Id90_HE10_IsoM:
                ...
                if entry.Photon_pt[photonindex] >= 130 and entry.Photon_pt[photonindex] < 175 and entry.HLT_Photon120_R9Id90_HE10_IsoM:
                    ps = GPr.get_value_from_yaml(
                        era, "HLT_Photon120_R9Id90_HE10_IsoM", entry.run, entry.luminosityBlock)
                else:
                    if entry.Photon_pt[photonindex] >= 175 and entry.HLT_Photon165_R9Id90_HE10_IsoM:
                        ps = GPr.get_value_from_yaml(
                            era, "HLT_Photon165_R9Id90_HE10_IsoM", entry.run, entry.luminosityBlock)
                    else:
                        ps = 0

    print('yaml', ps, entry.luminosityBlock, entry.run)
    return ps
...
def findSingleLeptTrigger(goodLeptonList, entry, flavour, era, printOn=False):
    ...
    if ('ee' in flavour or flavour == 'enu') and nLepton > 0:
        ...
        try:
            HLT_Ele25_eta2p1_WPTight_Gsf = entry.HLT_Ele25_eta2p1_WPTight_Gsf
        except AttributeError:
            HLT_Ele25_eta2p1_WPTight_Gsf = False
        try:
            HLT_Ele27_eta2p1_WPTight_Gsf = entry.HLT_Ele27_eta2p1_WPTight_Gsf
        except AttributeError:
            HLT_Ele27_eta2p1_WPTight_Gsf = False
        try:
            HLT_Ele32_WPTight_Gsf = entry.HLT_Ele32_WPTight_Gsf
        except AttributeError:
            HLT_Ele32_WPTight_Gsf = False
        try:
        ...
        try:
            HLT_Ele27_WPTight_Gsf = entry.HLT_Ele27_WPTight_Gsf
        except AttributeError:
            HLT_Ele27_WPTight_Gsf = False
        ...
        if 'ee' in flavour and nLepton < 2:
            return LepttrigList, hltList, hltListSubL
        if 'enu' in flavour and nLepton < 1:
            return LepttrigList, hltList, hltListSubL
        ...
        # if era != '2016' and entry.Electron_pt[goodLeptonList[0]] < 37 and
        # entry.Electron_pt[goodLeptonList[1]] < 37 : return LepttrigList,
        # hltList
    ...
    if (flavour == 'mm' or flavour == 'mnu') and nLepton > 0:
        ...
        try:
            HLT_IsoMu24 = entry.HLT_IsoMu24
        except AttributeError:
            HLT_IsoMu24 = False
        try:
            HLT_IsoMu27 = entry.HLT_IsoMu27
        except AttributeError:
            HLT_IsoMu27 = False

        try:
            HLT_IsoMu22 = entry.HLT_IsoMu22
        except AttributeError:
            HLT_IsoMu22 = False

        try:
            HLT_IsoMu22_eta2p1 = entry.HLT_IsoMu22_eta2p1
        except AttributeError:
            HLT_IsoMu22_eta2p1 = False

        try:
            HLT_IsoTkMu22 = entry.HLT_IsoTkMu22
        except AttributeError:
            HLT_IsoTkMu22 = False

        try:
            HLT_IsoTkMu24 = entry.HLT_IsoTkMu24
        except AttributeError:
            HLT_IsoTkMu24 = False

        try:
            HLT_IsoTkMu22_eta2p1 = entry.HLT_IsoTkMu22_eta2p1
        except AttributeError:
            HLT_IsoTkMu22_eta2p1 = False

        # print HLT_IsoMu24 , HLT_IsoMu27, HLT_IsoTkMu22, HLT_IsoTkMu24,
        # nLepton

        # print ord(entry.Tau_idDeepTau2017v2p1VSmu[j]), ord(entry.Tau_idDeepTau2017v2p1VSe[j]), ord(entry.Tau_idDeepTau2017v2p1VSjet[j])
        # if era == '2016' and not HLT_IsoMu22 and not HLT_IsoMu22_eta2p1 and
        # not HLT_IsoTkMu22 and not HLT_IsoTkMu22_eta2p1 and not HLT_IsoMu24
        # and not HLT_IsoTkMu24:  return LepttrigList, hltList, hltListSubL
        if '2016' in era and not HLT_IsoMu24 and not HLT_IsoTkMu24:
            return LepttrigList, hltList, hltListSubL
        # if era != '2016' and not HLT_IsoMu24 and not HLT_IsoMu27 :  return
        # LepttrigList, hltList, hltListSubL
        if '2016' not in era and not HLT_IsoMu27 and not HLT_IsoMu24:
            return LepttrigList, hltList, hltListSubL
        if 'mm' in flavour and nLepton < 2:
            return LepttrigList, hltList, hltListSubL
        if 'mnu' in flavour and nLepton < 1:
            return LepttrigList, hltList, hltListSubL
    ...
    # single mu 2018: HLT IsoMu24 v, HLT IsoMu27 v and cut pt(mu)>25,
    # eta(mu)<2.4
    dR = 100.
    dRr = 100.
    hltList = []
    ...
    for iobj in range(0, entry.nTrigObj):
        ...
        if ('ee' in flavour) and abs(entry.TrigObj_id[iobj]) == 11:
            ...
            if entry.TrigObj_filterBits[iobj] & 2 > 0:
                isbit2 = True
            if entry.TrigObj_filterBits[iobj] & 8 > 0:
                isbit8 = True
        ...
        if ('mm' in flavour) and abs(entry.TrigObj_id[iobj]) == 13:
            ...
            # print 'HLT_? ', HLT_IsoMu24, HLT_IsoMu27, entry.Muon_pt[leadL],
            # abs(entry.Muon_eta[leadL]), isbit2
        ...
        if dR < 0.5:
            ...
            if  (flavour == 'mm' or flavour == 'mnu'):
                ...
                if entry.Muon_pt[leadL] > 26 and abs(entry.Muon_eta[leadL]) < 2.4:
                    if '2016'  in era :
                        if (HLT_IsoMu24 and isbit2) or  (HLT_IsoTkMu24 and isbit8):
                            hltList.append(True)
                    if '2017' in era :
                        if (HLT_IsoMu27 and isbit2):
                            hltList.append(True)
                    if '2018' in era :
        ...
        if dRr < 0.5:
            ...
            if  (flavour == 'mm' or flavour == 'mnu'):
                ...
                if entry.Muon_pt[subleadL] > 26 and abs(entry.Muon_eta[subleadL]) < 2.4:
                    if '2016' in era :
                        if (HLT_IsoMu24 and isbit2) or  (HLT_IsoTkMu24 and isbit8):
    ...
    if len(hltList) > 0:
        LepttrigList.append(leadL)
    if len(hltListSubL) > 0:
        LepttrigList.append(subleadL)

    # if flavour=='ee'  :  print '--------------->',  era, flavour,
    # LepttrigList, 'hltL', hltList, 'hltSubL', hltListSubL,
    # entry.luminosityBlock, entry.event
    return LepttrigList, hltList, hltListSubL
...
class cutCounter():
    ...
    def printSummary(self):
        ...
        for nn in self.nickNames:
            fraction = 1.0
            if nLast > 0.:
                fraction = self.counter[nn] / nLast
            nLast = float(self.counter[nn])
            sFrac = '   N/A'
            if fraction < 1.0:
                sFrac = "{0:6.1f}%".format(100. * fraction)
            if fraction < 0.01:
                sFrac = "{0:6.3f}%".format(100. * fraction)
...
def getOutFileName(args):
    print(("In generalFunctions.getOutFileName() args.outFileName={0:s}".format(
        args.outFileName)))
    if len(args.outFileName) > 2:
        return args.outFileName
    if ('cmseos.fnal' in args.inFileName) or (
            'cms-xrd-global' in args.inFileName):
...
class dupeDetector():
    ...
    def printSummary(self):
        print(("Duplicate Event Summary: Calls={0:d} Unique Events={1:d}".format(
            self.nCalls, len(self.runEventList))))
        return
...
class pileUpWeight():
    ...
    def calculateWeights(self, nickName, year):
        ...
        fData = TFile('pileup_{0:s}UL_data.root'.format(year))
        ...
        MCfile = "pileup_{0:s}UL_MC.root".format(year)
        ...
        # hMC = fMC.Get('hMC_{0:s}'.format(nickName)) ##this is you would need
        # to have one histo per process ## old naming , new is the
        # Pileup_nTrueInt
        # this is you would need to have one histo per process
        hMC = fMC.Get('hPileup_nTrueInt')

        # hMC = fMC.Get('h{0:s}'.format(nickName)) ##this is you would need to have one histo per process
        # hMC = fMC.Get('pileup') ##this is you would need to have one histo
        # per process

        # check to be sure that data and MC histograms are commensurate
        if hData.GetBinWidth(1) != hMC.GetBinWidth(1) or hData.GetBinLowEdge(
                1) != hMC.GetBinLowEdge(1) or hData.GetNbinsX() != hMC.GetNbinsX():
        ...
        # print '========', self.PU, hMC.FindBin(PU), hData.FindBin(PU),
        # hData.GetBinContent(hData.FindBin(PU))/hMC.GetBinContent(hMC.FindBin(PU)),
        # 'is it the same?', weights
        xMin = hData.GetBinLowEdge(1)
    ...
    def getWeight(self, PU):
        ...
        try:
            # the sampleWeight should be taken from the MC later on
            weight = self.PUweights[iPU]
        except IndexError:
            weight = self.PUweights[98]
        # print 'weights', iPU, weight, self.sampleWeight, self.PUweights[iPU],
        # 'old', self.sampleWeight*self.PUweights[iPU]
        return weight
...
def findLast(e, ID, parent):
    ...
    for i in range(parent + 1, e.nGenPart, 1):
        if e.GenPart_pdgId[i] == ID:
            last = i
    if last < 0:
        print(("In generalFunction.findLast() particle not found: ID={0:d} parent={1:d}".format(
            ID, parent)))
...
class checkJSON():
    ...
    def printJSONsummary(self):
        print(("check JSON summary:  nCalls={0:d} nGood={1:d} nBad={2:d}".format(
