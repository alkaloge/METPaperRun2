...
if args.dataType.lower() == 'data':
    MC = False
if args.dataType.lower() == 'mc':
    MC = True
isMC = MC
...
if args.unique != 'none':
    ...
    print(
        ("******* Analyzing only {0:d} events from {1:s} ******.".format(len(uniqueEvents), args.unique)))
...
for count, e in enumerate(inTree):
    ...
    # print len(goodMuonList), goodMuonList, goodElectronList,
    # len(goodElectronList), cat
    for cat in catss:
        lepMode = cat[:2]
        if lepMode == 'ee':
        ...
        # print '========!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!this is odd....',
        # lepList, 'cat=====', catev, 'lepList_2', lepList_2, 'mass',
        # 'mass_2', M_2el, M_2mu , 'extraEl==========>',
        # len(goodElectronListExtraLepton), 'extraMu=========>',
        # len(goodMuonListExtraLepton)

        if not tauFun.mllCut((LepP + LepM).M()):
            continue

        if LepP.Pt() > LepM.Pt() and LepP.Pt() < 25:
            continue
        if LepM.Pt() > LepP.Pt() and LepM.Pt() < 25:
            continue

        if lepMode == 'ee':
            cutCounter[cat].count('FoundZ')
            if MC:
                cutCounterGenWeight[cat].countGenWeight('FoundZ', e.genWeight)
        if lepMode == 'mm':
            cutCounter[cat].count('FoundZ')
            if MC:
                cutCounterGenWeight[cat].countGenWeight('FoundZ', e.genWeight)
...
try:
    ...
    print(("Run time={0:.2f} s".format(dT)))
