...
outTuple = outTupleHZZ2L2Nu.outTupleHZZ2L2Nu(outFileName,  era, doSyst=True, isW=True)
...
for count, e in enumerate(inTree) :
    ...
    for cat in catss: 
	    ...
	    if MC : cutCounterGenWeight[cat].countGenWeight('GoodLeptons', e.genWeight)
            ...
            pairList, lepList = tauFun.findZHZZ2L2Nu(goodElectronList,[], e)
	    ...
	    if  MC :   cutCounterGenWeight[cat].countGenWeight('GoodLeptons', e.genWeight)
            ...
            pairList, lepList = tauFun.findZHZZ2L2Nu([],goodMuonList, e)
            if len(lepList) <2 : continue
        ...
        #print 'pt', (LepP+LepM).Pt()
        if (LepP+LepM).Pt() < 55 : continue
        if lepMode == 'ee' :
	    cutCounter[cat].count('ZPtgt55')
	    if  MC :   cutCounterGenWeight[cat].countGenWeight('ZPtgt55', e.genWeight)
        if lepMode == 'mm' :
	    cutCounter[cat].count('ZPtgt55')
	    if  MC :   cutCounterGenWeight[cat].countGenWeight('ZPtgt55', e.genWeight)

        if (LepP+LepM).M() > 91.1876 +15. : continue
        if (LepP+LepM).M() < 91.1876 -15. : continue

        if lepMode == 'ee' :
	    cutCounter[cat].count('Zwindow')
	    if  MC :   cutCounterGenWeight[cat].countGenWeight('Zwindow', e.genWeight)
        if lepMode == 'mm' :
	    cutCounter[cat].count('Zwindow')
	    if  MC :   cutCounterGenWeight[cat].countGenWeight('Zwindow', e.genWeight)
	...
	if not MC : isMC = False
        ...
        #doJME=False
        if len(lepList)<2 : continue
	outTuple.FillHZZ2L2Nu(e,catev,LepP,LepM,lepList,isMC,era,doJME, proc)
