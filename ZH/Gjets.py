...
if args.dataType == 'Data' or args.dataType == 'data' or args.dataType == '1' : MC = False
...
if nentries > 0:
	...
	if not tauFun.goodPhotonTrigger(e, args.year) : continue
	...
	goodPhotonList = tauFun.makeGoodPhotonList(e)
	vetoMuons=[]
	vetoElectrons=[]
	selecJets=[]
	...
	for cat in catss: 
	    ...
	    if lepMode == 'gjets' :
                ...
                #if len(goodPhotonList) >0 : print len(goodPhotonList), goodPhotonList
		if len(goodPhotonList) !=1 :continue
		cutCounter[cat].count('GoodPhotons')

                # print 'goodPhoton', goodPhotonList, len(goodPhotonList)
                #if len(goodPhotonList) !=1 or e.Photon_pt[goodPhotonList[0]] < 50 : continue
		if MC : cutCounterGenWeight[cat].countGenWeight('GoodPhotons', e.genWeight)

		
		for i in range(e.nMuon):
		    if  e.Muon_pt[i]>=10. and e.Muon_looseId[i] : vetoMuons.append(i)

		for j in range(e.nElectron):
		    if  e.Electron_pt[j]>=10. and e.Electron_cutBased[j] : vetoElectrons.append(j)

		
		if len(vetoMuons) > 0 or len(vetoElectrons) > 0 : continue
		cutCounter[cat].count('VetoLeptons')
		if MC : cutCounterGenWeight[cat].countGenWeight('VetoLeptons', e.genWeight)

		#for k in range(e.nJet):
		#    if e.Jet_pt[k]>=40. and abs(e.Jet_eta[k])<2.5 and e.Jet_jetId[k] == 6 : selecJets.append(k)
	       

		#if len(selecJets)<1 : continue
		#cutCounter[cat].count('nJets')
		#if MC : cutCounterGenWeight[cat].countGenWeight('nJets', e.genWeight)
	    ...
	    #	print 'fail nPhon', e.nPhoton
            #	continue
            if goodPhotonList[0] < 0 : continue
