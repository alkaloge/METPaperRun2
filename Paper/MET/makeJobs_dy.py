...
allsyst = [
    ...
    "JESUp",    "JESDown",
    "JERUp",    "JERDown",
    "UnclusteredUp",    "UnclusteredDown",
    "PUUp",    "PUDown",
    "IDUp",    "IDDown"]
#allsyst = ["Nominal"]
samples = ["data", "dy", "dynlo", "top", "ew"]
samples = ["dynlo", "top", "ew"]
...
#alljetcuts = ["njets_lpt_leta_geq0", "njets_lpt_leta_gt0"]
#alljetcuts += ["njets_hpt_leta_geq0", "njets_hpt_heta_gt0"]
        ...
        for yr in years:
            ...
            for ss in samples:
                ...
                if not OneByOne:
                    for ch in channels:
                        for vr in vars:
                            script_file_path = 'Jobs_dy' + IDWP + '/jobb_' + \
                            ...
                            for sy in allsyst:
                                ...
                                if 'data' in ss and 'Down' in sy:
                                    continue
                                if ('mll' in vr or 'boson_pt' in vr or 'Photon' in vr or 'Raw' in vr or 'iso_1' in vr or 'ignificance' in vr) and (
                                ...
                                with open(script_file_path, 'a') as script_file:
                                    command = 'python3 dy_met_distribution.py -y ' + \
                    ...
                    for ch in channels:
                        for vr in vars:
                            for sy in allsyst:
                                ...
                                if sy == "Nominal":
                                    sy = ""
                                if ('Photon' in vr or 'Raw' in vr or 'iso_1' in vr or 'ignificance' in vr or 'mll' in vr or 'boson_pt' in vr) and (
                                        'JE' in sy or 'Uncl' in sy):
                                    continue
...
if len(tosend) > 0:
    ...
    for i in tosend:
        ...
        if not os.path.isfile(
                '{0:s}/{1:s}.submitted'.format('Jobs_dy' + IDWP, i)):
