...
if __name__ == "__main__":
    ...
    fInB = fInC = fInD = None
    fInFit = TFile.Open('/uscms_data/d3/alkaloge/MetStudies/nAOD/CMSSW_11_3_4/src/Alexei/fitDiagnostics_wjets.root', 'read')
    ...
    puname=''
    data_hist_nom = fIn.Get('histo_data')
    data_gr = fInFit.Get('shapes_fit_b/wjets/data')

    data_hist = fInFit.Get('shapes_fit_b/wjets/total_background')
    data_hist.Reset()
    for i in range(data_gr.GetN()):
        x = r.Double(0)
        y = r.Double(0)
        ...
        error = data_gr.GetErrorY(i)
        bin_index = data_hist.FindBin(y)
        data_hist.SetBinError(bin_index, error)

    for ib in range(1,  data_hist_nom.GetNbinsX() + 1): print  'Comparing data ib', ib, data_hist_nom.GetBinContent(ib), data_hist_nom.GetBinError(ib),data_hist.GetBinContent(ib), data_hist.GetBinError(ib) 
    print data_hist_nom.Integral(), data_hist.Integral()
    #data_hist.Sum2()
    varTitle    = 'p_{T}^{miss} [GeV]'
    if 'boson_pt' in givein: varTitle    = 'W q_{T} [GeV]'
    if 'boson_phi' in givein : varTitle    = 'W #phi'
    ...
    if 'METCorGoodboson_transm' in givein: varTitle    = 'm_{T} [GeV]'
    if '_transm' in givein: varTitle    = 'm_{T} [GeV]'
    if 'boson_m' in givein and 'mt' not in givein: varTitle    = 'W mass [GeV]'
    if 'PuppiMET' in givein :  # and '_pt' in givein : varTitle = 'PUPPI-p_{T}^\rm{miss} [GeV]'
    ...
    if 'Raw' in givein :  varTitle = 'Raw-'+varTitle
    ...
    for v in varbs :
        ...
        for i in range(0, len(samples)):
            ...
            sumA=0.            
            sumB=0.            
            print 'will read', "histo_" + s + "_" + v
            histo = fInFit.Get("shapes_fit_b/wjets/histo_"+s+"_"+v)
            histoPre = fInFit.Get("shapes_prefit/wjets/histo_"+s+"_"+v)
            print histo.Integral(), histo.GetName()
	    temp = data_hist.Clone(data_hist.GetName())
            #histo = histoPre
	    temp.Reset()
            print 'binssssssss', temp.GetNbinsX(), histo.GetNbinsX()
	    for i in range(1, temp.GetNbinsX() + 1):
		temp.SetBinContent(i, histo.GetBinContent(i))
		temp.SetBinError(i, histo.GetBinError(i))
		temp.GetXaxis().SetTitle( data_hist.GetXaxis().GetTitle())
		temp.GetYaxis().SetTitle(data_hist.GetYaxis().GetTitle())

                sumA+=histo.GetBinContent(i)
                sumB+=histoPre.GetBinContent(i)
                print 'postFit', histo.GetBinContent(i), histo.GetName(), temp.GetBinContent(i), 'prefit', histoPre.GetBinContent(i), sumA, sumB, sumA-sumB
            hMC_total.Add(temp)
	...
	histo = fInFit.Get("shapes_fit_b/wjets/histo_" + s + "_" + v)
	histoPre = fInFit.Get("shapes_prefit/wjets/histo_" + s + "_" + v)
        #create a clone of the histo with the nominal binning
	temp = data_hist.Clone(data_hist.GetName())
        temp.Reset()
	temp.GetXaxis().SetTitle( data_hist.GetXaxis().GetTitle())
	temp.GetYaxis().SetTitle(data_hist.GetYaxis().GetTitle())
        for i in range(1, temp.GetNbinsX() + 1):
            temp.SetBinContent(i, histo.GetBinContent(i))
            temp.SetBinError(i, histo.GetBinError(i))
            print 'again  ', i , histo.GetBinContent(i), histoPre.GetBinContent(i), histo.GetNbinsX(), histoPre.GetNbinsX(), histo.GetName()
    ...
    for v in varbs :
	    ...
	    histoPost = fInFit.Get("shapes_fit_b/wjets/histo_" + s + "_" + v)
            histo.Reset()
	    for i in range(1, data_hist.GetNbinsX() + 1):
                histo.SetBinContent(i, histoPost.GetBinContent(i))
                histo.SetBinError(i, histoPost.GetBinError(i))
    ...
    for ilog in logcase :
        isLog = ilog
	plot_var = Canvas.Canvas("test/paperv2/%s_%s%s%s%s_doQCD_%s%s_%sLog_postFit"%(str(opts.varr),puname, channel, puname ,str(era), str(int(doQCD)), str(opts.ExtraTag), str(isLog)), "png,root,pdf,C", leg[0], leg[1], leg[2], leg[3])               
