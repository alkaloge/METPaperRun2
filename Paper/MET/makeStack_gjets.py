...
def create_relative_contribution_stack(input_stack):
    ...
    relative_stack = r.THStack(
        "relative_stack",
        "Stacked Histograms (Relative)")
    ...
    for hist in histograms:
        # Clone the original histogram to create a relative contribution
        # histogram
        ...
        for bin in range(1, relative_hist.GetNbinsX() + 1):
            ...
            if total_bin_content != 0.000:
                relative_contribution = hist_bin_content / total_bin_content
            else:
                relative_contribution = 0.000
            relative_hist.SetBinContent(bin, relative_contribution)
            # print "Histogram %d, Bin %d: Relative Contribution = %.4f" %
            # (i+1, bin, relative_contribution), hist.GetName(),
            # hist.GetTitle()
...
if __name__ == "__main__":
    ...
    # colors = {'dy':"#f89c20", 'dynlo':"#f89c20", 'qcd':"#832db6",
    # 'top':"#3f90da", 'ew':"#e42536",
    # 'ewknlo':"#92dadd",'ewknlo61':"#92dadd"}
    ...
    doStat = True
    doSyst = True
    if 'boson_pt' in givein or 'Raw' in givein or 'mll' in givein or 'iso_1' in givein or 'Photon_' in givein or 'hoe' in givein:
        doSyst = False
    ...
    for sample in samples:
        ...
        print("{:.3f}".format(contribution).ljust(max_length + 5), end=' ')

    # Print the total MC, data, and data/MC ratio
    # Print the total MC, data, and data/MC ratio with two significant digits
    print("{:.3f}\t".format(hMC_total.Integral()).ljust(max_length + 5), end=' ')
    print("{:.3f}\t".format(data_hist.Integral()).ljust(max_length + 5), end=' ')
    ...
    for v in varbs:
        ...
        for i in range(0, len(samples)):
            s = str(samples[i])
            histo = fIn.Get("histo_" + s + "_" + v)
            if 'NLO' in histo.GetTitle():
                histo.SetTitle(histo.GetTitle().replace("(NLO)", ""))
            if 'Gamma' in histo.GetTitle():
                histo.SetTitle(histo.GetTitle().replace("Gamma", "#gamma"))
            if kFactor != 1.:
                histo.Scale(kFactor)
            try:
    ...
    if '_pu' in str(opts.ExtraTag).lower():
        mc_stack_norm = create_relative_contribution_stack(mc_stack)
	...
	plot_varrL.saveRatioGjets(1,1, isLog, lumi, data_hist, mc_histo,  mc_jerup, mc_jerdown, mc_jesup, mc_jesdown, mc_unclup, mc_uncldown, mc_puup, mc_pudown, mc_idup, mc_iddown, varTitle , option+"_norm", run_str)
       ...
       '''
        ...
        isLog = 1
        plot_varr = Canvas.Canvas(
            "test/paperv2/%s_%s%s%s%s_doQCD_%s%s_%sLog" % (str( opts.varr), puname, channel, puname, str(era), str( int(doQCD)), str( opts.ExtraTag), str(isLog)), "png,root,pdf,C", leg[0], leg[1], leg[2], leg[3])
