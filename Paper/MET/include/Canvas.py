...
class Canvas:
    ...
    def __init__(self, name, format, x1, y1, x2, y2):
        ...
        if ":" in self.plotNames:
            plotNAmes = plotNames.replace(":", "_vs_")
        ...
        extraa = ''
        if 'METCor' in name :
            self.var = 'PF T1 p_{T}^{miss}'
        if 'MET_p' in name:
            self.var = 'PF p_{T}^{miss}'
        if 'MET_T1' in name:
            self.var = 'PF T1 p_{T}^{miss}'
        if 'PuppiMET_p' in name:
            self.var = 'PUPPI p_{T}^{miss}'

        if 'Smear' in name:
            self.var = 'PF T1 Smear p_{T}^{miss}'
        if 'PuppiMETCor' in name:
            self.var = 'PUPPI T1 p_{T}^{miss}'

        if 'Raw' in name and 'MET' in self.var:
            self.var = 'Raw ' + str(self.var)
        if 'Raw' in name:
            if 'puppi' in name.lower():
                self.var = 'Raw PUPPI p_{T}^{miss}'
            if 'puppi' not in name.lower() and 'met' in name.lower():
                self.var = 'Raw PF p_{T}^{miss}'

        # str(self.var) = str(self.var).replace("[]","")

        if 'Mu' in name:
            self.extra = 'W #rightarrow #mu#nu'
        if 'El' in name:
            self.extra = 'W #rightarrow e#nu'
        if 'dy' in name and 'Mu' in name:
            self.extra = 'Z #rightarrow #mu#mu'
        if 'dy' in name and 'El' in name:
            self.extra = 'Z #rightarrow ee'
        if 'gjets' in name:
            self.extra = '#gamma+jets'
        ...
        if 'isvbf' in name:
            if 'njetsgt0' in name or 'eta_gt0' in name:
                self.extra = "#splitline{" + self.extra + "}{VBF}"
        if 'isnotvbf' in name:
        ...
        if 'vbf' not in name:
            if 'njetseq0' in name or 'eta_eq0' in name:
                if '_pu' not in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}=0}"
                if '_pult10' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0  - nVtx<10}"
                if '_pu10to20' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0 - 10<nVtx<20}"
                if '_pu20to30' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0 - 20<nVtx<30}"
                if '_pu30to40' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0 - 30<nVtx<40}"
                if '_pu40to50' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0 - 40<nVtx<50}"
                if '_pugeq50' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0 - nVtx>50}"
                if '_pugeq40' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=0 - nVtx>40}"
            if 'njetseq1' in name or 'eta_eq1' in name:
                if '_pu' not in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}=1}"
                if '_pult10' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - nVtx<10}"
                if '_pu10to20' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - 10<nVtx<20}"
                if '_pu20to30' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - 20<nVtx<30}"
                if '_pu30to40' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - 30<nVtx<40}"
                if '_pu40to50' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - 40<nVtx<50}"
                if '_pugeq50' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - nVtx>50}"
                if '_pugeq40' in name:
                    self.extra = "#splitline{" +  self.extra + "}{N_{jets}=1 - nVtx>40}"
            if 'njetsgt0' in name or 'eta_gt0' in name:
                if '_pu' not in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}>0}"
                if '_pult10' in name:
            ...
            if 'njetsgeq0' in name or 'eta_geq0' in name:
                if '_pu' not in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}#geq0}"
                if '_pult10' in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}#geq0 - nVtx<10}"
                if '_pu10to20' in name:
                    self.extra = "#splitline{" + self.extra +  "}{N_{jets}#geq0 - 10<nVtx<20}"
                if '_pu20to30' in name:
                    self.extra = "#splitline{" + self.extra +  "}{N_{jets}#geq0 - 20<nVtx<30}"
                if '_pu30to40' in name:
                    self.extra = "#splitline{" + self.extra +  "}{N_{jets}#geq0 - 30<nVtx<40}"
                if '_pu40to50' in name:
                    self.extra = "#splitline{" + self.extra +  "}{N_{jets}#geq0 - 40<nVtx<50}"
                if '_pugeq50' in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}#geq0 - nVtx>50}"
                if '_pugeq40' in name:
                    self.extra = "#splitline{" + self.extra + "}{N_{jets}#geq0 - nVtx>40}"

        if 'lpt' in name:
            if 'leta' in name:
                self.extra = self.extra.replace( "N_{jets}", "pT<50, |#eta|<2.4, N_{jets}")
            if 'heta' in name:
                self.extra = self.extra.replace( "N_{jets}", "pT<50, |#eta|>2.4, N_{jets}")
        if 'hpt' in name:
            if 'leta' in name:
        ...
        if B in name:
            self.extra = self.extra + " B- iso<.15, W_{T}<80 "
        if C in name:
            self.extra = self.extra + " C- iso>.15, W_{T}>80 "
        if D in name:
            self.extra = self.extra + " D- iso>.15, W_{T}<80 "
...
#      latexc.DrawLatex(0.6, 0.55, chisquare)OA
        ...
        for i in range(0, len(self.histos)):
            for j in range(0, len(self.orderForLegend)):
                if(self.orderForLegend[j] != -1 and self.orderForLegend[j] == i):
                    ...
                    self.myLegend.AddEntry(
                        self.histos[j], self.labels[j], self.labelsOption[j])
        # else:
    ...
    def saveRatio2( self, legend, isData, log, lumi, hdata, hMC, hjerUp, hjerDown, hjesUp, hjesDown, hunclUp, hunclDown, title, option, run='2016', r_ymin=0, r_ymax=2): 
        ...
        lTex2v = TLatex()
        lTex2v.SetNDC()
        lTex2v.SetTextSize(0.045)
        lTex2v.SetTextFont(42)
        lTex2v.DrawLatex(0.4, 0.75, '{0:s}'.format(self.var))

        lTex2 = TLatex()
        lTex2.SetNDC()
        lTex2.SetTextSize(0.045)
        lTex2.DrawLatex(0.28, 0.75, '{0:s}'.format(self.extra), 'same')
        ...
        for i in range(ratio.GetNbinsX()):
            ...
            uncl_down = hunclDown.GetBinContent(
                i + 1) - hMC.GetBinContent(i + 1)

            jes_uncertainty = (jes_up**2 + jes_down**2)**0.5
            jer_uncertainty = (
                jer_up**2 +
                jer_down**2 +
                jes_uncertainty**2)**0.5
            uncl_uncertainty = (
                uncl_up**2 +
                uncl_down**2 +
                jer_uncertainty**2 +
            ...
            uncl_band.SetPointError(
                i, 0, 0, uncl_uncertainty, uncl_uncertainty)
            '''
	...
	for i in range(jes_band.GetN()):
        ...
        jer_band.SetFillColor(r.TColor.GetColor("#964a8b"))
        jer_band.SetFillStyle(1001)
        jes_band.SetFillColor(r.TColor.GetColor("#92dadd"))
        jes_band.SetFillStyle(1001)
        syst_band.SetFillColor(r.TColor.GetColor("#b9ac70"))
        ...
        if doAllErrors:
            ...
            ratio.Draw()
            colors = [
                r.TColor.GetColor("#964a8b"),
                r.TColor.GetColor("#b9ac70"),
                r.TColor.GetColor("#964a8b"),
                r.TColor.GetColor("#92dadd"),
                r.kMagenta]
            ...
            # Redraw the ratio histogram on top to keep the axis and markers
            # visible
            ratio.Draw("same")
    ...
    def saveRatio( self, legend, isData, log, lumi, hdata, hMC, hjerUp, hjerDown, hjesUp, hjesDown, hunclUp, hunclDown, title, option, run='2016', r_ymin=0, r_ymax=2): 
        ...
        # hjesDown ,hunclUp, hunclDown', hdata.GetSumOfWeights(),
        ...
        lTex2v.SetTextSize(0.045)
        lTex2v.SetTextFont(42)
        lTex2v.DrawLatex(0.28, 0.75, '{0:s}'.format(str(self.var)))
        ...
        for km in range(0, hMC.GetNbinsX() + 1):
            ...
            if conte1tottot > conte2tottot:
                ...
                # print 'in the else will add errors tottot', km,
                # hMC.GetBinContent (km), conte2tottot
        ...
        for km in range(0, hMC.GetNbinsX() + 1):
            ...
            if not (conte1tot + conte2tot) > 0:
                continue
            if (abs(conte1tot - conte2tot) / (conte1tot + conte2tot)
                    < 1) and (conte1tot + conte2tot) > 0:
                if conte1tot > conte2tot:
                    den1tot.SetBinContent(
                        km, hMC.GetBinContent(km) + conte1tot)
                    den2tot.SetBinContent(
                        km, hMC.GetBinContent(km) - conte1tot)
                else:
                    den1tot.SetBinContent(
                        km, hMC.GetBinContent(km) + conte2tot)
                    den2tot.SetBinContent(
                        km, hMC.GetBinContent(km) - conte2tot)
            else:
                if conte1tot > conte2tot:
        ...
        for km in range(0, hMC.GetNbinsX() + 1):
            ...
            conte1 = math.sqrt(hMC.GetBinError(
                km)**2 + (hjesUp.GetBinContent(km) - hMC.GetBinContent(km))**2)

            conte2 = math.sqrt(hMC.GetBinError(
                km)**2 + (hMC.GetBinContent(km) - hjesDown.GetBinContent(km))**2)
        ...
        errtottot.SetFillColor(r.TColor.GetColor("#964a8b"))
        errtottot.SetFillStyle(1001)
        errtot.SetFillColor(r.TColor.GetColor("#964a8b"))
        errtot.SetFillStyle(1001)

        err.SetFillColor(r.TColor.GetColor("#92dadd"))
        err.SetFillStyle(1001)
        staterr.SetFillColor(r.TColor.GetColor("#b9ac70"))
    ...
    def saveRatioGjets(self, legend, isData, log,  lumi, hdata, hMC, hjerUp, hjerDown ,  hjesUp, hjesDown ,hunclUp, hunclDown, hpuUp, hpuDown, hidUp, hidDown,title,  option, run = '2016',  r_ymin=0, r_ymax=2):
        ...
        lumi  = str(formatted_lumi)
        #plot_var.saveRatio(1,1, isLog, lumi, data_hist, mc_histo, mc_up, mc_down, mc_jesup, mc_jesdown, mc_unclUp, mc_unclDown, varTitle , option, run_str)
        #print('inside Canvas' 'hdata, hMC, hjerUp, hjerDown ,  hjesUp, hjesDown ,hunclUp, hunclDown', hdata.GetSumOfWeights(), hMC.GetSumOfWeights(), hjerUp.GetSumOfWeights(), hjerDown.GetSumOfWeights(), hjesUp.GetSumOfWeights(), hjesDown.GetSumOfWeights(), hunclUp.GetSumOfWeights(), hunclDown.GetSumOfWeights(), hpuUp.GetSumOfWeights(), hpuDown.GetSumOfWeights(), hpuUp.GetBinContent(13)/hMC.GetBinContent(13), hpuUp.GetBinContent(13)/hMC.GetBinContent(13))


        #for i in range(hMC.GetNbinsX()+1):
        ...
        #if 'phi' in title.lower() : 
        #    hMC.SetMaximum(hMC.GetMaximum()*200)
        #    hdata.SetMaximum(hMC.GetMaximum())
        ...
        if hMC.GetSumOfWeights() != hpuUp.GetSumOfWeights() and hMC.GetSumOfWeights() == hpuDown.GetSumOfWeights():
            doSomeErrors = 1
        #    statOnly = 1
        if hMC.GetSumOfWeights() != hjesUp.GetSumOfWeights() and hMC.GetSumOfWeights() != hjesDown.GetSumOfWeights():
            doAllErrors = 1
        ...
        for km in range(1, hMC.GetNbinsX()+1):
            ...
            #if hMC.GetBinContent(km) > 0 : print (' from iD and PU diff', km, puErr, idErr, 'hMC', hMC.GetBinContent(km), 'puUp', hpuUp.GetBinContent(km)/hMC.GetBinContent(km), 'puDown', hpuDown.GetBinContent(km)/hMC.GetBinContent(km))

            if hMC.GetBinContent(km) > 0. : hMC.SetBinError(km, math.sqrt(binE**2 + lumiErr**2 + puErr**2 + idErr**2+leptErr**2 + trigErr**2))
            #if hMC.GetBinContent(km) > 0. : hMC.SetBinError(km, math.sqrt(binE**2 + lumiErr**2 ))
            #hMC.SetBinError(km, math.sqrt(binE*binE + lumiErr*lumiErr ))#+ leptErr*leptErr + trigErr*trigErr))
        ...
        for i in range(0, len(self.histos)):
            if(self.ToDraw[i] != 0):
                ...
                if 'phi' in hMC.GetName().lower() and log: 
                    ...
                    self.histos[i].SetMaximum(hMC.GetMaximum()*5000)
                if ('boson_transm' in hMC.GetName().lower() or 'boson_pt' in hMC.GetName().lower()) and log: 
                    #print ('this is MAX-------------------------------------------------------------', title.lower())
                    self.histos[i].SetMaximum(hMC.GetMaximum()*5000)
                if 'dr' in hMC.GetName().lower() and log: 
                    print ('this is MAX', title.lower())
                    self.histos[i].SetMaximum(hMC.GetMaximum()*1000)
                       
                #print ('--------------SETMAX --------------========================= histo i', i, title, hMC.GetName(), hMC.GetMaximum(), 'histos name', self.histos[i].GetName(), self.histos[i].GetMaximum(), log, lowAxis, setLowAxis, setUpAxis)
        ...
        if puppi: 
            ...
            line.Draw("same")
            ratio.Draw("same");      
        if statOnly and not doAllErrors:
            #legratio = TLegend(0.14,0.32,0.43,0.5);
            #legratio.SetFillColor(0);
            #legratio.SetBorderSize(0);
            #legratio.AddEntry(staterr, " ","f");                                  
