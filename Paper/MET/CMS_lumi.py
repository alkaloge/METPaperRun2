    ...
    if( not outOfFrame ):
        if( drawLogo ):
            ...
            if( writeExtraText ) :
                ...
                laatex.DrawLatex(posX_, posY_- relExtraDY*cmsTextSize*t, extraText)
                #latex.DrawLatex(posX_, posY_, extraText)
    elif( writeExtraText ):
        if( iPosX==0):
            posX_ =   l +  relPosX*(1-l-r)
            posY_ =   1 +lumiTextOffset*t

        latex.SetTextFont(extraTextFont)
        latex.SetTextSize(extraTextSize*t)
        latex.SetTextAlign(align_)
        latex.DrawLatex(posX_+t, posY_, extraText + extraText2)      
