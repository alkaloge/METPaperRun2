...
samples = ['data', 'dy', 'qcd', 'top', 'ew', 'ewknlo61', 'ewkincl',  'ewk']
...
#year=str(sys.argv[1])
year='2018'
#plotS_2018_njetseq0_nbtagl_cutbased_varbins_vetolept_isolt0p15_mtmassgt80_hitslt1_METCorGood_T1_pt_ElNu.root
#for var in vars:
if True:
    ...
    #root_file = ROOT.TFile("plotS_"+year+"_njetseq0_nbtagl_cutbased_varbins_vetolept_isolt0p15_mtmassgt80_hitslt1_"+var+"_"+channel+".root")
    #root_file=ROOT.TFile("plotS_2016postVFP_njetsgt0_nbtagl_cutbased_varbins_vetolept_isolt0p15_mtmassgt80_hitslt1_METCorGood_T1_pt_ElNu.root")
    #root_file=ROOT.TFile("plotS_2018_njetsgt0_nbtagl_cutbased_openbins_vetolept_isolt0p15_mtmassgt80_hitslt1_PuppiMET_phi_ElNu.root")
    root_file_path = sys.argv[1]
    # Open the ROOT file
    root_file = ROOT.TFile(root_file_path)
    # Extract the filename from the path
    filename = root_file_path.split('/')[-1]

    # Define the start and end markers
    start_marker = "hitslt1_"
    end_markers = ["_ElNu", "_MuNu"]

    # Find the position of the start marker
    start_pos = filename.find(start_marker) + len(start_marker)

    # Determine which end marker is present in the filename
    end_pos = len(filename)
    for marker in end_markers:
        marker_pos = filename.find(marker)
        if marker_pos != -1:
            end_pos = marker_pos
            break

    # Extract the substring between the markers
    var = filename[start_pos:end_pos]
    ...
    for sample in samples:
        ...
        if 'data' in sample :sname= 'histo_data'
        ...
        if nominal_hist : 
            ...
            #row.append(nominal_hist.GetBinContent(13))
            ...
            for syst in systs:
                for variation in variations:
                    ...
                    if syst_hist and syst_hist.GetSumOfWeights() : # Add the systematic SumOfWeight to the row
                        ...
                        #row.append(nominal_hist.GetBinContent(13)/syst_hist.GetBinContent(13))
                    else : 
                        row.append("-")
                        bug=str(year+"_"+svar)
