...
if __name__ == "__main__":
    ...
    fInFit = TFile.Open('/uscms_data/d3/alkaloge/MetStudies/nAOD/CMSSW_11_3_4/src/fitDiagnosticsdatacards.root', 'read')
    fInFit = TFile.Open('/uscms_data/d3/alkaloge/MetStudies/nAOD/CMSSW_11_3_4/src/Alexei/fitDiagnostics_gjets.root', 'read')
    #fInFit.cd("shapes_fit_b/Gjets/")
...
if 'njets' in givein : varTitle    = 'njets'
    ...
    print 'some infoooooooooooooooooooooooo', data_hist.Integral(), data_hist.GetName()
    fInFit.ls()
    ...
    for v in varbs :
        ...
        for i in range(0, len(samples)):
            ...
            histo = fInFit.Get("shapes_fit_b/Gjets/histo_"+s+"_"+v)
            histoPre = fInFit.Get("shapes_prefit/Gjets/histo_"+s+"_"+v)
    ...
    for v in varbs :
	    ...
	    histoPost = fInFit.Get("shapes_fit_b/Gjets/histo_" + s + "_" + v)
            histo.Reset()
    ...
    if '_pu' in  str(opts.ExtraTag).lower() : 
        if PUnorm  : 
	    plot_varr = Canvas.Canvas("test/paperv2/%s_%s%s%s%s_doQCD_%s%s_%sLog_norm_postFit"%(str(opts.varr),puname, channel, puname ,str(era), str(int(doQCD)), str(opts.ExtraTag), str(isLog)), "png,root,pdf,C", leg[0], leg[1], leg[2], leg[3])               
	...
	isLog = 1
	plot_varr = Canvas.Canvas("test/paperv2/%s_%s%s%s%s_doQCD_%s%s_%sLog_postFit"%(str(opts.varr),puname, channel, puname ,str(era), str(int(doQCD)), str(opts.ExtraTag), str(isLog)), "png,root,pdf,C", leg[0], leg[1], leg[2], leg[3])               
