...
void graph_scale(const std::string& vs="_vspt_", const std::string& year="2018", const std::string& channel="MuMu", const std::string& dy_="dy", const std::string& njet_="eq0"){
  //setTDRStyle();
  ...
  //std::string channel = "MuMu";
  //std::string vs = "_vspt_"; //_npv_
  //vs = "_npv_"; //_npv_
  std::string common_part = "_alldata"+vs+"njets"+njet_+"_";
  std::string common_partmc = "_"+dy_+vs+"njets"+njet_+"_";

 
  //  rawmet_2018_dy"+vs+"njetsgeq0_MuMu.txt
  //"t1_2018_alldata_npv_njetsgeq0_2018.txt
  std::ifstream frawd("txt/rawmet_" + year + common_part + channel + ".txt");
  std::ifstream frawpd("txt/rawpuppi_" + year + common_part + channel + ".txt");
  std::ifstream ft1d("txt/t1_" + year + common_part + channel + ".txt");
  std::ifstream fpuppid("txt/puppi_" + year + common_part + channel + ".txt");
  std::ifstream fpuppinomcd("txt/puppi_" + year + common_part + channel + "_noMC.txt");

  std::ifstream frawmc("txt/rawmet_" + year+  common_partmc + channel + ".txt");
  std::ifstream frawpmc("txt/rawpuppi_" + year + common_partmc + channel + ".txt");
  std::ifstream ft1mc("txt/t1_" + year+ common_partmc + channel + ".txt");
  std::ifstream ft1smc("txt/t1smear_" + year+  common_partmc + channel + ".txt");
  std::ifstream fpuppimc("txt/puppi_" + year+ common_partmc + channel + ".txt");


  cout <<"working on ..." + year + common_part + channel + ".txt"<<"  "<<common_partmc<<endl;
  TString saveName ="scale_"+ year +dy_+"_"+ "njets"+njet_ +vs+channel+".pdf";
  TString saveNamepar ="res_par_"+ year +dy_+ "_" + "njets"+njet_ +vs+channel+".pdf";
  TString saveNameperp ="res_perp_"+ year +dy_+ "_" + "njets"+njet_ +vs+channel+".pdf";
...
if (vs == "_vspt_")
{
    ...
    //Float_t vspt_bins[] = {0., 20., 40., 60., 80., 100., 120., 160., 200., 300.};
...
//par
 ...
 //perp
  ...
  grrawd->GetXaxis()->SetRangeUser(0, 300);
  if (vs =="_npv_") { 
