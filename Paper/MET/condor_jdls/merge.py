...
#if 'Gjets' in channels : Njet=['geq1', 'incl']
...
#MC = ['GJets_HT-40To100', 'GJets_HT-100To200', 'GJets_HT-200To400','GJets_HT-400To600','GJets_HT-600ToInf']
#MC = ['QCD_HT1000to1500MG',    'QCD_HT100to200MG' ,     'QCD_HT1500to2000MG',    'QCD_HT2000toInfMG' ,    'QCD_HT200to300MG',        'QCD_HT300to500MG' ,    'QCD_HT500to700MG' ,    'QCD_HT50to100MG' ,    'QCD_HT700to1000MG' , 'WJetsToLNu_NLO']

MCdy = ['DYJetsToLLM50NLO']
...
if command.lower() == "merge" or command.lower()=='step1': 
	...
	for channel in channels:
		...
		if channel == "Gjets" :
                    ...
                    if '2018' not in year and 'preV' not in year : os.system("hadd -f scales_SinglePhoton_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SinglePhoton_Run{0}*root".format(year, channel, njet))
                    if 'preV' in year : os.system("hadd -f scales_SinglePhoton_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SinglePhoton_Run*root".format(year, channel, njet))
                    for mc in MC : 
                        os.system("hadd -f scales_{3}_{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_{3}_*{1}.root".format(year, channel, njet,mc))
                        os.system("python merge_histos.py {0} {1} {2} {3}".format(mc, year, njet, channel))
		    ...
		    if channel == "ElEl" : 
			if  "2017" in year or year=='2016' :os.system("hadd -f scales_SingleElectron_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleElectron_Run{0}*root".format(year, channel, njet))
			if year == "2016preVFP"  :os.system("hadd -f scales_SingleElectron_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleElectron_Run2016*pre*root".format(year, channel, njet))
	...
	for channel in channels:
		...
		if channel == "Gjets" : 
		    if year == "2018" :
                        ...
                        os.system("hadd -f weighted_scales_MC_{0}_{1}_Njet{2}.root weighted_mc_scales_QCD_HT*_{0:s}_{1}_Njet{2}.root  weighted_mc_scales_WJetsToLNu_NLO*_{0:s}_{1}_Njet{2}.root".format(year, channel, njet))
                        #os.system("hadd -f weighted_scales_MC_{0}_{1}_Njet{2}.root weighted_mc_scales_QCD_HT*_{0:s}_{1}_Njet{2}.root ".format(year, channel, njet))
                        os.system("python fitDDGausian.py scales_EGamma_Run{0}_{1}_Njet{2}.root weighted_scales_MC_{0}_{1}_Njet{2}.root {0} {1} {2}; cp *png fit_result*pdf /publicweb/a/alkaloge/plots/MetStudiesPaper/Scales/; ".format(year, channel, njet))
                    ...
                    if year != "2018" :
                        ...
                        os.system("hadd -f weighted_scales_SinglePhoton_Run{0}_{1}_Njet{2}.root scales_SinglePhoton_Run{0}_{1}_Njet{2}.root weighted_mc_scales_QCD_HT*_{0:s}_{1}_Njet{2}.root".format(year, channel, njet))
                        os.system("hadd -f weighted_scales_MC_{0}_{1}_Njet{2}.root weighted_mc_scales_QCD_HT*_{0:s}_{1}_Njet{2}.root  weighted_mc_scales_WJetsToLNu_NLO*_{0:s}_{1}_Njet{2}.root".format(year, channel, njet))
                        os.system("python fitDDGausian.py scales_SinglePhoton_Run{0}_{1}_Njet{2}.root weighted_scales_MC_{0}_{1}_Njet{2}.root {0} {1} {2}; cp *png fit_result*pdf /publicweb/a/alkaloge/plots/MetStudiesPaper/Scales/; ".format(year, channel, njet))
		    ...
		    if channel == "MuMu" and 'pre' not in year: os.system(" python fitDDGausian.py scales_SingleMuon_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleMuon_Run{0}*root".format(year, channel, njet))
		    if channel == "MuMu" and 'pre' in year: os.system("python fitDDGausian.py scales_SingleMuon_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleMuon_Run2016*pre*root".format(year, channel, njet))
		    if channel == "MuMu" : 
			#if year == "2017" or year=='2016' :os.system("hadd -f scales_SingleMuon_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleMuon_Run{0}*root".format(year, channel, njet))
			#if year == "2016preVFP"  :os.system("hadd -f scales_SingleMuon_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleMuon_Run2016*pre*root".format(year, channel, njet))
			#if year == "2018" :os.system("hadd -f scales_SingleMuon_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleMuon_Run{0}*root".format(year, channel, njet))
			os.system("hadd -f scales_SingleMuon_Run{0}_{1}_Njet{2}.root {1}_{0}/scales_Njet{2}_all_SingleMuon_Run*root".format(year, channel, njet))
...
if command.lower() == "extract" or command.lower() == 'step3': 
	...
	for channel in channels:
		...
		if "Gjets" not in channel: 
		    #if channel == "MuMu" and 'pre' not in year : os.system("python extract2.py scales_SingleMuon_Run{0}_{1}_Njet{2}.root {0} {1} {1:s} ".format(year, channel, njet))
		    if channel == "MuMu"  : os.system("python extract2.py scales_SingleMuon_Run{0}_{1}_Njet{2}.root {0} {1} {1:s} ".format(year, channel, njet))
		    if channel == "ElEl" : 
			if "2018" not  in year :os.system("python extract2.py scales_SingleElectron_Run{0}_{1}_Njet{2}.root {0} {1} {1:s}".format(year, channel, njet))
			else :os.system("python extract2.py scales_EGamma_Run{0}_{1}_Njet{2}.root {0} {1} {1:s}".format(year, channel, njet))
		    #os.system("python extract2.py scales_DYJetsToLLM50_{0}_{1}_Njet{2}.root {1} {0}".format(year, channel, njet))
		    os.system("python extract2.py weighted_mc_scales_DYJetsToLLM50NLO_{0}_{1}_Njet{2}.root {0} {1} {1:s}".format(year, channel, njet))
