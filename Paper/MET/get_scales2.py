...
def get_scales():
    ...
    if channel == "ElEl":
        ...
        jcut = " ( event.nElectron==2 and event.Flag_BadPFMuonDzFilter==1  and   ( math.fabs(event.eta_1)<=1.4442 or math.fabs(event.eta_1)>=1.5660) and math.fabs(event.d0_1)<0.045  and  math.fabs(event.dZ_1)<0.2 and  event.q_1==1  and  event.iso_1 <= .15  and   (math.fabs(event.eta_2)<=1.4442 or math.fabs(event.eta_2)>=1.5660)   and  math.fabs(event.d0_2)<0.045  and  math.fabs(event.dZ_2)<0.2  and   math.fabs(event.q_2)==1  and  event.iso_2 <= .15  and event.nPVGood>2  and  event.cat==1   and event.njets{0:s}    and  event.Electron_convVeto_1 > 0  and  event.Electron_lostHits_1<1 and event.Electron_convVeto_2 > 0  and  event.Electron_lostHits_2<1 )".format(str(sjets))

 #( nElectron[0]==2 && Flag_BadPFMuonDzFilter[0]==1  &&  !(fabs(eta_1[0])>1.4442 &&  fabs(eta_1[0])<1.5660) && fabs(d0_1[0])<0.045 && fabs(dZ_1[0])<0.2 && fabs(q_1[0])==1 && iso_1[0] <= .15 &&   !(fabs(eta_2[0])>1.4442 &&  fabs(eta_2[0])<1.5660) && fabs(d0_2[0])<0.045 && fabs(dZ_2[0])<0.2 &&  fabs(q_2[0])==1 && iso_2[0] <= .15 && nPVGood[0]>2 && cat==1 && njets[0] >0  && Electron_convVeto_1[0] > 0 && Electron_lostHits_1[0]<1 && Electron_convVeto_2[0] > 0 && Electron_lostHits_2[0]<1 && iso_1[0] <=0.1 && iso_2[0] <=0.1)
    ...
    if channel == "Gjets" : 
        ...
        jcut = " (   event.pt_1>=50 and math.fabs(event.eta_1)<1.44 and event.Flag_BadPFMuonDzFilter==1 and event.nPVGood>2 and event.njets{0:s}  and event.Photon_r9_1>=0.9 and event.Photon_r9_1<=1. and event.iso_1<=0.005 and event.Photon_cutBased_1 ==3 )".format(str(sjets))
