...
#vars=["METCorGood_T1Smear_pt", "METCorGood_T1_pt", "PuppiMETCorGood_pt", "RawMET_pt", "RawPuppiMET_pt", "boson_pt", "u_parboson_METCorGood_T1", "u_parboson_METCorGood_T1Smear", "u_parboson_PuppiMETCorGood", "u_parboson_RawMET", "u_parboson_RawPuppiMET", "u_perp_METCorGood_T1", "u_perp_METCorGood_T1Smear", "u_perp_PuppiMETCorGood", "u_perp_RawMET", "u_perp_RawPuppiMET"]

#vars+=["METCorGood_T1_pt", "METCorGood_T1Smear_pt", "PuppiMETCorGood_pt"]
...
#vars= [var for var in vars if "Smear"  in var]
...
#for pu in allpv : 
	...
	if 'iso_' in vars or 'Photon_hoe' in vars : njets = nj+'_nobtag_' + ID + "_varbins_cutbasedtight"+pu
	...
	for yr in years:
		...
		for var in vars:
		    ...
		    if yr =='2016' : hadd_command = "hadd -f {0:s} {5:s}/plotS_2016preVFP_{1:s}_hitslt{2:s}_{3:s}_{4:s}.root {5:s}/plotS_2016postVFP_{1:s}_hitslt{2:s}_{3:s}_{4:s}.root ".format(fileIn, njets, veto, var, ch, rootDir)
		    if yr =='Run2' : hadd_command = "hadd -f {0:s} {5:s}/plotS_2016preVFP_{1:s}_hitslt{2:s}_{3:s}_{4:s}.root {5:s}/plotS_2016postVFP_{1:s}_hitslt{2:s}_{3:s}_{4:s}.root {5:s}/plotS_2017_{1:s}_hitslt{2:s}_{3:s}_{4:s}.root {5:s}/plotS_2018_{1:s}_hitslt{2:s}_{3:s}_{4:s}.root".format(fileIn, njets, veto, var, ch, rootDir)
		    print hadd_command
		    ...
		    if not doPost : os.system("python3 makeStack_gjets.py -f {} -y {} -v {} -q 0 -e {} -c {} -l no".format(fileIn, yr, var, extraTag, ch))
		    else : os.system("python3 makePostFit_gjets.py -f {} -y {} -v {} -q 0 -e {} -c {} -l no".format(fileIn, yr, var, extraTag, ch))
		    ...
		    #${var}_gjets${yr}_doQCD_0${extraTag}_0Log.pdf
                    ...
                    if doPost :                    
			os.system("cp plots/test/paperv2/{0:s}_gjets{1:s}_doQCD_0{2:s}_0Log_postFit.pdf /publicweb/a/alkaloge/plots/MetStudiesPaper/Gjv4/{0:s}_{1:s}_doQCD0_{2:s}_noLog_Gjets_postFit.pdf".format(var, yr, extraTag))

			os.system("cp plots/test/paperv2/{0:s}_gjets{1:s}_doQCD_0{2:s}_1Log_postFit.pdf /publicweb/a/alkaloge/plots/MetStudiesPaper/Gjv4/{0:s}_{1:s}_doQCD0_{2:s}_Log_Gjets_postFit.pdf".format(var, yr, extraTag))

			if '_pu' in extraTag : 
			    os.system("cp plots/test/paperv2/{0:s}_gjets{1:s}_doQCD_0{2:s}_0Log_norm_postFit.pdf /publicweb/a/alkaloge/plots/MetStudiesPaper/Gjv4/{0:s}_{1:s}_doQCD0_{2:s}_noLog_norm_Gjets_postFit.pdf".format(var, yr, extraTag))

			    os.system("cp plots/test/paperv2/{0:s}_gjets{1:s}_doQCD_0{2:s}_1Log_norm_postFit.pdf /publicweb/a/alkaloge/plots/MetStudiesPaper/Gjv4/{0:s}_{1:s}_doQCD0_{2:s}_Log_norm_Gjets_postFit.pdf".format(var, yr, extraTag))
                    else:
