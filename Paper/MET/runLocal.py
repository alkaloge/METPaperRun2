...
command="python makeAllPlotsZHCondorWithSyst.py -f {2:s}_{0:s}_ZH_pow_noL.txt -a ZH -s {7:s} --MConly  -y {0:s}  -r no -w 16 -j {1:s} -e pow_noL -i {4:s} -t {3:s} -b {5:s} -x {6:s} -X {8:d}".format(str(year), syst, dataset, nickName, islocal, btag, subrange,sign, parts)
